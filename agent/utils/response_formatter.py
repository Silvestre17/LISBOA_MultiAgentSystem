...
def structure_weather_markdown(text: str) -> str:
    ...
    for idx, raw_line in enumerate(source_lines):
        ...
        if _is_detail_line(stripped):
            # Use a flat list style for weather details to keep visual spacing
            # consistent across Streamlit markdown renderings.
...
def canonicalize_visitlisboa_source_line(
    ...
    elif kind == "events" and not _has_researcher_place_hint(query_lower):
...
def canonicalize_planner_source_line(text: str, language: str = "en") -> str:
    ...
    if (
        "ipma" in lower_body
        or re.search(r"\b(weather|rain|showers|temperature|temperatures|vento|chuva|meteorolog)\b", lower_body)
    ):
        sources.append("[*IPMA*](https://www.ipma.pt)")

    if (
        "metrolisboa" in lower_body
        or "metro de lisboa" in lower_body
        or re.search(r"\bmetro\b", lower_body)
    ):
...
def format_researcher_card(text: str, language: str = "en", user_query: str = "") -> str:
    ...
    for raw_line in lines:
        ...
        start_match = _RESEARCHER_CARD_START_RE.match(stripped) or _BOLD_RESEARCHER_CARD_START_RE.match(stripped)
...
def finalize_worker_response(
    ...
    if agent_name == "weather":
        ...
        elif researcher_kind != "events" and not already_structured_place_cards:
...
def normalize_researcher_card_field_indentation(text: str) -> str:
    ...
    field_re = re.compile(
        ...
        r"More details|Mais detalhes|Tickets|Bilhetes|Date/Time|Data/Hora|Duration|Duração):\*\*",
        re.IGNORECASE,
    )
    non_card_titles = {
        "planning evidence",
    ...
    def _is_card_heading(stripped: str) -> bool:
        match = card_heading_re.match(stripped) or bold_card_heading_re.match(stripped)
        if not match:
            return False
        title = _strip_accents_compat(_strip_markdown_formatting(match.group("title"))).lower().strip(" .:-")
        return title not in non_card_titles
    ...
    for raw_line in text.splitlines():
        ...
        if stripped.startswith("### ") or stripped.startswith("**"):
            in_card = _is_card_heading(stripped)
            output_lines.append(raw_line)
            continue
...
def normalize_planner_transport_section_indentation(text: str) -> str:
    """Keep movement bullets nested inside planner transport sections."""
    if not text:
        return text or ""
    ...
    field_re = re.compile(r"^[-*]\s+(?:📍|🟡|🔵|🟢|🔴|🚇|🚌|🚋|🚆|🚶|⏱️|🗺️)\s+")
    for raw_line in text.splitlines():
        stripped = raw_line.strip()
        if stripped.startswith("### "):
            in_transport_section = bool(re.match(r"^###\s+🚇\s+", stripped))
            output_lines.append(raw_line)
            continue
        if _SOURCE_LINE_RE.match(stripped):
            in_transport_section = False
            output_lines.append(raw_line)
            continue
        if in_transport_section and field_re.match(stripped):
            output_lines.append(f"    - {stripped.lstrip('-* ').strip()}")
            continue
        output_lines.append(raw_line)
    return "\n".join(output_lines)
...
def repair_route_value_bold_markers(text: str) -> str:
    ...
    return route_label_re.sub(_repair, text)
...
def final_visual_pass(text: str) -> str:
    ...
    def _merge_inferred_sources(value: str) -> str:
        """Merge inferred evidence-based sources into the existing footer.

        The merge is conservative: it only adds a source when the response body
        explicitly mentions the corresponding data family and when a source footer
        already exists.
        """
        lines = value.splitlines()
        source_lines = [line.strip() for line in lines if _SOURCE_LINE_RE.match(line.strip())]
        if not source_lines:
            return value

        footer_body = "\n".join(source_lines)
        body_without_source = "\n".join(
            line for line in lines if not _SOURCE_LINE_RE.match(line.strip())
        )
        lowered = body_without_source.lower()
        lower_footer = footer_body.lower()

        language = "pt" if re.search(r"\b(?:Fonte|Fontes|Atualizado)\b", footer_body) else "en"
        is_pt = language == "pt"
        timestamp = extract_update_time(value) or datetime.now().strftime("%H:%M")
        sources: list[str] = [line for line in re.findall(r"\[[^\]]+\]\([^)]+\)", footer_body)]
        existing_source_urls: set[str] = set(
            source.split("](", 1)[1].rstrip(")").lower().rstrip("/") for source in sources
            if "](" in source
        )
        added_any = False
        ...
        if re.search(r"\b(weather|temperatura|temperature|chuva|rain|wind|vento|aviso|warning|warnings)\b", lowered):
            _append_source(
                "https://www.ipma.pt",
                "[*IPMA*](https://www.ipma.pt)",
            )

        if re.search(r"\b(metrolisboa|metro de lisboa|metro)\b", lowered) and re.search(
            r"\b(line|linha|intercamb|intercâmbio|estação|transfer|circul|vermelha|azul|verde|amarela|red|blue|green|yellow)\b",
            lowered,
        ):
            _append_source(
        ...
        if re.search(r"\b(carris)\b", lowered):
            _append_source(
                "https://www.carris.pt",
                "[*Carris*](https://www.carris.pt)",
            )

        if re.search(r"\b(carrismetropolitana|aml|suburban)\b", lowered):
            _append_source(
                "https://www.carrismetropolitana.pt",
        ...
        if re.search(r"\b(visitlisboa|museu|museum|mosteiro|monastery|monumento|monument|cultural|café|cafe|miradouro|viewpoint|jardim|garden|palace|palácio|castelo|castle)\b", lowered):
            ...
            if "visitlisboa.com" not in lower_footer:
                _append_source(
                    "https://www.visitlisboa.com",
                    source_key,
                )

        if not sources and not added_any:
            return value

        if re.search(r"\b(ecoponto|reciclagem|municipal|farm[áa]cia|farmacia|pharmacy|estacionamento|parking|mercado|market|biblioteca|library|hospital|pol[ií]cia)\b", lowered):
            _append_source(
                "https://dados.cm-lisboa.pt",
                "[*Lisboa Aberta*](https://dados.cm-lisboa.pt/)",
            )

        replacement = (
            f"📌 **Fonte:** {' | '.join(sources)} | **Atualizado:** {timestamp}"
            if is_pt
            else f"📌 **Source:** {' | '.join(sources)} | **Updated:** {timestamp}"
        )
        # Keep all distinct source links discovered in the body and footer.
        return _replace_source_line(
            value,
            replacement,
    ...
    def _strip_duplicate_semantic_bullets(value: str) -> str:
        ...
        for line in value.splitlines():
            ...
            if stripped.startswith("###") or stripped == "---" or is_bold_card_heading:
    ...
    text = re.sub(r"(?m)^-\s*🚇\s+Movement Logic\s*$", "### 🚇 Movement Logic", text)
    text = re.sub(r"(?m)^-\s*🚇\s+Lógica de transporte\s*$", "### 🚇 Lógica de transporte", text)
    ...
    text = re.sub(r"(?ms)^###\s+🌤️\s+Weather Snapshot\s*\n+\s*---\s*\n+", "", text)
    ...
    text = _fix_lisboa_aberta_only_source_footer(text)
    text = strip_internal_repository_source_links(text)
    text = text.replace("**⛅ Conditions and Rain Strategy**", "### ⛅ Conditions and Rain Strategy")
    text = text.replace("**⛅ Condições e estratégia**", "### ⛅ Condições e estratégia")
    text = text.replace("**🚇 Movement Logic**", "### 🚇 Movement Logic")
    text = text.replace("**🚇 Lógica de transporte**", "### 🚇 Lógica de transporte")
...
def _normalize_researcher_visual_contract(text: str, language: str) -> str:
    ...
    if language == "pt":
        ...
        value = re.sub(r"https://www\.visitlisboa\.com/en/events\b", "https://www.visitlisboa.com/pt-pt/eventos", value)
        value = re.sub(r"https://www\.visitlisboa\.com/en/places\b", "https://www.visitlisboa.com/pt-pt/locais", value)
        value = re.sub(r"https://www\.visitlisboa\.com/en/restaurants\b", "https://www.visitlisboa.com/pt-pt/restaurantes", value)
...
def normalize_planner_item_card_indentation(text: str) -> str:
    """Restore nested field indentation inside planner route cards.

    Args:
        text: Markdown response after final visual cleanup.

    Returns:
        Markdown with item-card fields nested under ``**🏷️ Item**`` blocks in
        planner suggested-route sections.
    """
    if not text:
        return text or ""
    ...
    in_item_card = False
    field_re = re.compile(r"^\s*[-*]\s+(?:📝|🏷️|📍|🕒|💶|🌐|🎟️|☔|⚠️)\s+")
