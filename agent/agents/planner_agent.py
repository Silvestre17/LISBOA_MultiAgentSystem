...
# Closure must mean "not usable now/today" (today-closed or renovation/maintenance),
# never a weekly closing day ("Encerrado: segunda-feira"), a lunch break
# ("fechado para almoco"), or an opening time ("closed until 10:00").
_PLANNER_UNUSABLE_CLOSED_RE = re.compile(
    r"\b(?:hoje fechado|fechado hoje|hoje encerrado|encerrado hoje|today closed|closed today|"
    r"closed for renovation|closed for refurbishment|closure until|"
    r"renovation until|temporarily closed|temporary closure|temporarily unavailable|"
    r"encerrad[oa]s? temporariamente|temporariamente encerrad[oa]s?|"
    r"(?:encerrad|fechad)[oa]s?\s+(?:para|por)\s+(?:obras|manutencao|renovacao|restauro|remodelacao)|"
    r"horario hoje fechado|hours today closed)\b"
    r"|\bclosed until\b(?!\s*\d{1,2}(?:\s*[:h]\s*\d{0,2}|\s+\d{2}|\s*[ap]\.?m\.?)?(?:\s|$))"
)
...
def _clean_requested_anchor_fragment(fragment: str) -> str:
    ...
    cleaned = re.sub(
        r"\s+(?:hoje|amanh[aã]|today|tomorrow|esta\s+(?:manha|manh[aã]|tarde|noite)|"
        r"this\s+(?:morning|afternoon|evening)|tenho|com\s+mobilidade|with\s+reduced|"
        r"quero\s+pouca|low\s+walking)\b.*$",
        "",
        cleaned,
        flags=re.IGNORECASE,
    )
...
def _split_requested_anchor_fragment(fragment: str) -> List[str]:
    ...
    for part in _PLANNER_ANCHOR_LIST_SPLIT_RE.split(str(fragment or "")):
        ...
        conjoined_parts = re.split(
            r"\s+(?:e|and)\s+(?=(?:(?:[oa]s?|the)\s+)?[A-ZÁÀÂÃÉÊÍÓÔÕÚÇ0-9])",
            cleaned_part,
        )
        if len(conjoined_parts) <= 1:
            ...
            continue
        expanded_parts = [
            # Strip only lowercase grammatical articles; capitalized articles are
            # part of official venue names ("Os Tibetanos", "A Brasileira").
            re.sub(r"^(?:os?|as?|the)\s+", "", candidate.strip(" .:-")).strip(" .:-")
            for candidate in conjoined_parts
            if candidate.strip(" .:-")
        ]
...
def _extract_requested_anchor_phrases(user_message: str) -> List[str]:
    ...
    endpoint_patterns = [
        r"\b(?:de|do|da|desde)\s+(?P<origin>[^,.;]+?)\s+(?:para|ate|até|a|ao|aos|as|à|às)\s+(?P<destination>[^.;]+)",
...
def _requested_anchor_time_constraints(user_message: str) -> List[tuple[str, str]]:
    ...
    patterns = [
        ...
        (
            r"\b(?P<place>almo[cç]o|lunch|jantar|dinner)\s+"
            r"(?:(?:às|as|at|pelas|por\s+volta\s+d(?:as?|e)?|around|by)\s+)"
            + time_re
        ),
    ]
...
def _requested_meal_time_constraints(user_message: str) -> List[tuple[str, str]]:
    """Extract meal-at-time constraints where no specific place is named.

    Handles patterns like "almoço às 13:00" or "lunch at 13:00" where the
    meal type itself is the anchor (not a named restaurant).
    """
    text = str(user_message or "")
    if not text.strip():
        return []
    time_re = r"(?P<time>\d{1,2}(?:[:hH]\d{0,2})?)"
    pattern = (
        r"\b(?P<meal>almo[cç]o|almoçar|almocar|lunch|jantar|dinner)\s+"
        r"(?:(?:às|as|at|pelas|por\s+volta\s+d(?:as?|e)?|around|by)\s+)"
        + time_re
    )
    ...
    for m in re.finditer(pattern, text, flags=re.IGNORECASE):
        meal = m.group("meal").lower()
        meal = re.sub(r"almo[cç]ar?", "almoço", meal)
        time_label = _normalize_requested_time_label(m.group("time"))
        if not time_label:
            continue
        key = f"{meal}|{time_label}"
        if key in seen:
            continue
        seen.add(key)
        results.append((meal, time_label))
    return results[:3]
...
_PLANNER_CENTRAL_AREA_RE = re.compile(
    ...
    r"santa luzia|portas do sol|bacalhoeiros|douradores|correeiros|"
    r"santa cruz do castelo)\b",
    re.IGNORECASE,
)
...
_LISBON_CITY_OUTSIDE_AREA_RE = re.compile(
    r"\b(?:sintra|cascais|queluz|oeiras|almada|costa\s+da\s+caparica|"
    r"palacio\s+da\s+pena|pena\s+palace|castelo\s+dos\s+mouros|moorish\s+castle|"
    r"palacio\s+nacional\s+de\s+queluz|palacio\s+de\s+queluz)\b",
    re.IGNORECASE,
)


def _query_requests_lisbon_city_only_plan(user_message: str) -> bool:
    """Return whether the user asked for a plan constrained to Lisbon city."""
    normalized = _normalize_planner_text(user_message)
    if not re.search(r"\b(?:lisboa|lisbon)\b", normalized):
        return False
    if re.search(
        r"\b(?:sintra|cascais|queluz|oeiras|almada|aml|area\s+metropolitana|"
        r"arredores|perto\s+de\s+lisboa|near\s+lisbon|around\s+lisbon|"
        r"day\s+trip|bate[-\s]?volta|fora\s+de\s+lisboa)\b",
        normalized,
    ):
        return False
    return bool(
        re.search(
            r"\b(?:plano|plan|roteiro|itinerario|itiner[aá]rio|dias?|days?|tarde|afternoon|manha|manh[aã])\b",
            normalized,
        )
    )


def _planner_card_is_outside_lisbon_city(card: Dict[str, str]) -> bool:
    """Return whether a card is clearly outside Lisbon city boundaries."""
    ...
    return bool(_LISBON_CITY_OUTSIDE_AREA_RE.search(basis))
...
def _planner_response_has_local_area_drift(response: str, user_message: str) -> bool:
    ...
    if _query_requests_lisbon_city_only_plan(user_message):
        route_text = _normalize_planner_text("\n".join(_planner_response_route_blocks(response)))
        if _LISBON_CITY_OUTSIDE_AREA_RE.search(route_text):
            return True
    ...
    area_allowed_re = {
        ...
        "marvila": re.compile(
            r"\b(?:marvila|beato|madre\s+de\s+deus|museu\s+nacional\s+do\s+azulejo|azulejo|"
            r"xabregas|poco\s+do\s+bispo|poco\s+bispo|praca\s+david\s+leandro\s+da\s+silva|1950|1900)\b"
        ),
...
def _enforce_walking_only_movement(response: str, user_message: str, language: str) -> str:
    ...
    for raw_line in str(response or "").splitlines():
        ...
        if in_movement and re.match(r"^[-*]\s+", stripped):
            normalized = _normalize_planner_text(stripped)
            if re.search(
                r"\b(?:deslocacao\s+a\s+pe\s+estimada|estimated\s+walking\s+segment|"
                r"no\s+exact\s+pedestrian\s+route|nao\s+tenho\s+dados\s+exatos\s+de\s+percurso\s+pedonal)\b",
                normalized,
            ):
                continue
            ...
            if has_transport_operator:
                continue
            raw_line = re.sub(r"^(\s*[-*]\s*)🚇", r"\1🧭", raw_line)
            kept_in_section = True
...
def _extract_planner_activity_titles(response: str) -> List[str]:
    """Extract stop/block titles from a Markdown planner draft.

    The legacy Markdown contract renders each itinerary stop as a standalone
    fully-bolded header line (``**📍 Place Name**``, optionally bulleted)
    inside the "Suggested Route"/"Roteiro Sugerido" section. Headings
    (``### ...``) and field bullets with trailing text never match because the
    pattern requires the line to end at the closing ``**``.

    Args:
        response: Raw planner draft in Markdown.

    Returns:
        Block titles with any leading emoji/symbol cluster stripped.
    """
    text = response or ""
    section_match = re.search(
        r"(?msi)^###\s+[^\n]*(?:Suggested Route|Roteiro Sugerido)[^\n]*\n(?P<body>.*?)(?=^###\s|\Z)",
        text,
    )
    scope = section_match.group("body") if section_match else text
    titles: List[str] = []
    for match in re.finditer(
        r"(?m)^(?!\s*#)\s*(?:[-*]\s+)?\*\*(?P<title>[^*\n]+?)\*\*\s*$",
        scope,
    ):
        title = re.sub(r"^[^\w(]+", "", match.group("title")).strip()
        if title:
            titles.append(title)
    return titles
...
def _find_planner_grounding_issues(
    ...
    activity_lines = _extract_planner_activity_titles(response)
...
def _compact_planner_context_block(
    ...
    *,
    ...
    for raw_line in str(content).splitlines():
        ...
        normalized_line = _normalize_planner_text(stripped)
        if re.search(
            r"\b(?:proximas partidas|partidas previstas|next departures|upcoming departures)\b",
            normalized_line,
        ):
            continue
        line = raw_line.rstrip()
        if len(line) > 420:
            compact_line = re.sub(r"\*\*", "", stripped)
            compact_line = re.sub(r"\s+", " ", compact_line).strip()
            cut_at = max(compact_line.rfind(". ", 0, 360), compact_line.rfind("; ", 0, 360))
            if cut_at >= 120:
                compact_line = compact_line[: cut_at + 1]
            else:
                compact_line = compact_line[:360].rstrip(" ,;:-") + "."
            line = compact_line

        if kept_lines and char_count + len(line) > max_chars:
            break
...
def _filter_planner_cards_for_request_constraints(
    ...
    if _query_requests_lisbon_city_only_plan(user_message):
        outside_city_cards = [
            ...
            if _planner_card_is_outside_lisbon_city(card)
        ]
        if outside_city_cards and len(outside_city_cards) < len(filtered):
            filtered = [card for card in filtered if card not in outside_city_cards]
...
def _planner_response_missing_requested_food_stop(response: str, user_message: str) -> bool:
    ...
    route_blocks = _planner_response_route_blocks(response)
    route_scope = "\n\n".join(route_blocks) if route_blocks else without_sources
    normalized_route_scope = _normalize_planner_text(route_scope)
    if re.search(
        ...
        has_visible_meal_placeholder = bool(
            re.search(
                r"\b(?:almoco|almoço|jantar|lunch|dinner|refeicao|refeição|meal)\b",
                normalized_response,
            )
        )
        requested_meal_times = _requested_meal_time_constraints(user_message)
        keeps_requested_meal_times = all(
            time_label in (response or "")
            for _meal_label, time_label in requested_meal_times
            if time_label
        )
        return not (has_visible_meal_placeholder and keeps_requested_meal_times)
    ...
    if len(explicitly_requested_meals) > 1:
        ...
        return any(
            not re.search(meal_patterns[meal_kind], normalized_route_scope)
            for meal_kind in explicitly_requested_meals
        )

    if _query_requests_custard_tart_stop(user_message):
        if re.search(r"\b(?:hoje\s+fechado|fechado\s+hoje|today\s+closed|closed\s+today)\b", normalized_response) and not re.search(
            r"\b(?:pastel(?:\s+de\s+nata)?|pasteis(?:\s+de\s+nata)?|nata|custard(?:\s+tarts?)?|tarts?|pastelaria|pastry)\b",
    ...
    if _query_requests_cafe_stop(user_message):
        if re.search(r"\b(?:hoje\s+fechado|fechado\s+hoje|today\s+closed|closed\s+today)\b", normalized_response) and not re.search(
            r"\b(?:cafe|coffee|pastelaria|pastry|brunch)\b",
            normalized_route_scope,
        ):
            return True
        return not bool(
    ...
    *,
        ...
        if strict_requested_sequence and requested_cards:
            ...
            food_candidates = sorted(
                [
                    ...
                    and _planner_food_card_allowed_for_requested_local_area(card, user_message)
                ],
    ...
    if visible_days == 1:
        ...
        for card in visible_cards:
            ...
            card_kind = _card_kind_for_plan_block(card)
            if card_kind == "food" and _query_requests_food_stop(user_message):
                normalized_display = _normalize_planner_text(display_name)
                requested_meals = _requested_meal_kinds(user_message)
                if "dinner" in requested_meals and not re.search(r"\b(?:jantar|dinner)\b", normalized_display):
                    display_name = f"{'Jantar' if is_pt else 'Dinner'}: {display_name}"
                elif "lunch" in requested_meals and not re.search(r"\b(?:almoco|almoço|lunch)\b", normalized_display):
                    display_name = f"{'Almoço' if is_pt else 'Lunch'}: {display_name}"
            icon = "🍽️" if card_kind == "food" else "🏛️"
        ...
        themes_en = [
            ...
            "heritage, museums, and traditional food",
            "indoor-friendly culture with a calm pace",
            "museums and viewpoints with short transfers",
            "flexible rainy backup and cheap-food day",
        ]
        themes_pt = [
            "Lisboa histórica com pouca caminhada",
            "património, museus e comida tradicional",
            "cultura com opção interior e ritmo calmo",
...
def _extract_requested_plan_area(user_message: str) -> str:
    ...
    patterns = [
        ...
        r"\b(?:termin\S*|acab\S*|finishing|ending|finish|end)\s+(?:perto\s+d(?:e|o|a|os|as)|em|no|na|near|around|at|in)\s+(?P<area>[^,.;]+?)(?:\s+(?:e|and|mant[eé]m|mantem|preserva|keep|keeping|com|with|passa|passar|via)\b|\s+no\s+segundo\s+dia|\s+on\s+day\s+\d+|[,.;]|$)",
        r"\b(?:termin\S*|acab\S*|finishing|ending|finish|end)\s+(?:(?:o|a|no|na|on|the)\s+)?(?:primeiro|segundo|terceiro|\d+)(?:\s+dia|\s+day)?\s+(?:perto\s+d(?:e|o|a|os|as)|em|no|na|near|around|at|in)\s+(?P<area>[^,.;]+?)(?:[,.;]|$)",
        r"\b(?:through|via|por(?!\s+volta\b)|pela|pelo)\s+(?P<area>[^,.;]+?)(?:\s+(?:with|com|sem|without|para|for)\b|[,.;]|$)",
...
def _planner_card_matches_area(card: Dict[str, str], area: str) -> bool:
    ...
    if normalized_area in {"marvila", "beato"}:
        return bool(
            re.search(
                r"\b(?:marvila|beato|madre\s+de\s+deus|museu\s+nacional\s+do\s+azulejo|azulejo|"
                r"xabregas|poco\s+do\s+bispo|poco\s+bispo|praca\s+david\s+leandro\s+da\s+silva|1950|1900)\b",
...
def _planner_consecutive_stop_transition_items(
    ...
    for origin_card, destination_card in zip(visible_cards, visible_cards[1:5]):
        ...
        if wants_public_transport:
            ...
            items.append(
                (
                    f"🚶 **{origin_name} → {destination_name}:** deslocação a pé estimada — "
                    "não tenho dados exatos de percurso pedonal entre estes pontos; consulte a distância no mapa antes de partir."
                )
                if is_pt
                else (
                    f"🚶 **{origin_name} → {destination_name}:** estimated walking segment — "
                    "no exact pedestrian route data available for this leg; check the distance on a map before you go."
                )
            )
    return items
...
def _query_requests_return_to_origin(user_message: str) -> bool:
    ...
    return bool(
        re.search(
            ...
            r".{0,80}\b(?:hotel|base|alojamento|accommodation|origem|origin|ponto de partida|"
            r"ponto inicial|inicio|starting point|there|la|ali)\b",
            normalized,
        )
        or re.search(
            r"\b(?:hotel|base|alojamento|accommodation|origem|origin|ponto de partida|"
            r"ponto inicial|inicio|starting point)\b"
...
_PLAN_ORIGIN_NON_PLACE_RE = re.compile(
    r"(?i)^(?:"
    r"autocarros?|bus(?:es)?|metro|metropolitano|comboios?|trains?|trams?|"
    r"el[eé]tricos?|eletricos?|barcos?|ferr(?:y|ies)|t[aá]xis?|uber|carros?|cars?|"
    r"bicicletas?|bikes?|a\s+p[eé]|p[eé]|foot|walking|"
    r"manh[aã]|tarde|noite|madrugada|morning|afternoon|evening|tonight|"
    r"hoje|amanh[aã]|today|tomorrow|"
    r"meio\s+dia|meio-dia|half\s+day|full\s+day|dia\s+inteiro|fim\s+de\s+semana|weekend|"
    r"\d+\s+(?:paragens?|stops?|museus?|museums?|dias?|days?|horas?|hours?|"
    r"minutos?|minutes?|pessoas?|people)|"
    r"museus?|museums?|monumentos?|monuments?|miradouros?|viewpoints?|"
    r"restaurantes?|restaurants?|igrejas?|churches|paragens?|stops?|"
    r"lugares|locais|places|s[ií]tios?|sites"
    r")$"
)


def _clean_plan_origin_candidate(origin: str) -> str:
    """Keep an extracted origin only when it can plausibly name a place.

    The broad "de X para/a Y" patterns also capture transport modes
    ("de autocarro até Belém"), times of day ("de manhã ao Chiado"), and
    quantities ("de 3 paragens até..."), which must never become a visible
    plan origin.
    """
    cleaned = re.sub(r"\s+", " ", str(origin or "")).strip(" .:-")
    cleaned = re.sub(
        r"(?i)^(?:esta\s+)?(?:manh[aã]|tarde|noite|morning|afternoon|evening)\s+(?:d[eoa]s?\s+|em\s+|no\s+|na\s+)?",
        "",
        cleaned,
    )
    cleaned = re.sub(
        r"(?i)^(?:de\s+)?(?:autocarro|bus|metro|comboio|train|tram|el[eé]trico|eletrico|barco|ferry|t[aá]xi|carro|bicicleta)\s+(?:d[eoa]s?\s+)",
        "",
        cleaned,
    )
    # Lowercase-only: a capitalized article is part of an official venue name
    # ("A Brasileira", "Os Tibetanos") and must be preserved.
    cleaned = re.sub(r"^(?:o|a|os|as)\s+", "", cleaned).strip(" .:-")
    if not cleaned:
        return ""
    if _PLAN_ORIGIN_NON_PLACE_RE.match(cleaned):
        return ""
    if re.search(r"(?i)\b(?:quero|queria|gostava|gosto|vou|vamos|preciso|want|like|need)\b", cleaned):
        return ""
    return cleaned
...
def _extract_requested_plan_origin(user_message: str) -> str:
    ...
    text = str(user_message or "").strip()
    patterns = [
        r"\b(?:come[cç](?:o|ar|a|e|amos|ando)|part(?:o|imos|indo|ir)|saio|sa[ií]mos|iniciar|iniciando|starting|start)\s+(?:perto\s+d(?:e|o|a|os|as)|junto\s+a|near|close\s+to|no|na|em|at|from|in|d[eoa]s?)\s+(?P<origin>[^,.;]+?)(?:[,.;]|\s+(?:para|ate|até|to)\b|\s+às\b|\s+as\b|\s+at\b|$)",
        r"\bfrom\s+(?P<origin>[^,.;]+?)\s+to\b",
        r"\b(?:de|do|da|desde)\s+(?P<origin>[^,.;]+?)\s+(?:para|ate|até|a|ao|aos|as|à|às)\b",
        ...
        r"\bdesde\s+(?:o|a|os|as)?\s*(?P<origin>[^,.;]+?)(?:[,.;]?\s+(?:faz|com|inclui|incluindo|para|e)\b|$)",
        ...
        r"\ba partir d(?:e|o|a|os|as)\s+(?P<origin>[^,.;]+?)(?:[,.;]?\s+(?:faz|com|inclui|incluindo|para|e)\b|$)",
    ...
    for pattern in patterns:
        ...
        if match:
            ...
            origin = _clean_plan_origin_candidate(origin)
            if 2 <= len(origin) <= 80:
                return origin
    return ""
...
def _build_multi_day_movement_guidance(
    *,
    ...
    generic_guidance = (
        [
            "🧭 **Entre paragens do mesmo dia:** agrupa cada dia por zona; deixa 20-30 min entre paragens próximas e valida a distância real no mapa antes de sair.",
            "🚇 **Entre zonas/dias:** usa transporte público ou uma alternativa externa apenas quando a mudança de zona for longa; não fixei linhas, paragens ou partidas sem evidência específica da ferramenta.",
        ]
        if is_pt
        else [
            "🧭 **Between same-day stops:** keep each day grouped by area; leave 20-30 min between nearby stops and check the real distance on the map before leaving.",
            "🚇 **Between areas/days:** use public transport or an external alternative only when the area change is long; I did not fix lines, stops, or departures without specific tool evidence.",
        ]
    )
    actionable_bullets = []
    ...
    if not re.search(
        ...
        return actionable_bullets + generic_guidance
...
def _build_card_based_renderer_fallback(
    *,
    ...
    target_area = _extract_compact_plan_area_anchor(user_message)
    if target_area and not _planner_area_is_broad_city(target_area):
        target_area_cards = [
            card for card in cards
            if _planner_card_matches_area(card, target_area)
        ]
        if target_area_cards:
            local_food_cards = [
                card for card in selected_cards
                if _card_kind_for_plan_block(card) == "food"
                and _planner_food_card_allowed_for_requested_local_area(card, user_message)
            ]
            selected_cards = _dedupe_planner_cards([*target_area_cards, *local_food_cards])
    ...
    for index, card in enumerate(visible_cards, start=1):
        ...
        if not requested_time_label and _card_kind_for_plan_block(card) == "food":
            for meal_label, meal_time_label in _requested_meal_time_constraints(user_message):
                normalized_meal = _normalize_planner_text(meal_label)
                if re.search(r"\b(?:almoco|lunch)\b", normalized_meal) or re.search(
                    r"\b(?:jantar|dinner)\b",
                    normalized_meal,
                ):
                    requested_time_label = meal_time_label
                    break
        ...
        if _card_kind_for_plan_block(card) == "food" and _query_requests_food_stop(user_message):
            normalized_title = _normalize_planner_text(block_title)
            requested_meals = _requested_meal_kinds(user_message)
            if "dinner" in requested_meals and not re.search(r"\b(?:jantar|dinner)\b", normalized_title):
                block_title = f"{'Jantar' if is_pt else 'Dinner'}: {block_title}"
            elif "lunch" in requested_meals and not re.search(r"\b(?:almoco|almoço|lunch)\b", normalized_title):
                block_title = f"{'Almoço' if is_pt else 'Lunch'}: {block_title}"
        if requested_time_label:
            block_title = re.sub(r"^\s*\d{1,2}:\d{2}\s*(?:[-\u00b7]|\u00b7)\s*", "", block_title).strip()
            if _card_kind_for_plan_block(card) == "food":
                normalized_title = _normalize_planner_text(block_title)
                for meal_label, _meal_time_label in _requested_meal_time_constraints(user_message):
                    normalized_meal_label = _normalize_planner_text(meal_label)
                    if (
                        re.search(r"\b(?:almoco|lunch)\b", normalized_meal_label)
                        and not re.search(r"\b(?:almoco|lunch)\b", normalized_title)
                    ):
                        block_title = f"{'Almoço' if is_pt else 'Lunch'}: {block_title}"
                        break
                    if (
                        re.search(r"\b(?:jantar|dinner)\b", normalized_meal_label)
                        and not re.search(r"\b(?:jantar|dinner)\b", normalized_title)
                    ):
                        block_title = f"{'Jantar' if is_pt else 'Dinner'}: {block_title}"
                        break
...
def _order_historic_food_cards(
    cards: List[Dict[str, str]],
    user_message: str = "",
) -> List[Dict[str, str]]:
    """Order historic-and-food fallback cards into a practical one-day flow.

    When the user named an explicit area (e.g. "em Marvila", "in Alcântara"), cards
    matching that area are ranked first regardless of the generic centro/Belém preference.
    """
    ...
    explicit_anchor = _normalize_planner_text(_extract_compact_plan_area_anchor(user_message))

    def _zone_score(card: Dict[str, str]) -> int:
        if explicit_anchor and _planner_card_matches_area(card, explicit_anchor):
            return -1
        basis = _normalize_planner_text(
...
def _select_planner_cards_for_request(cards: List[Dict[str, str]], user_message: str) -> List[Dict[str, str]]:
    ...
    if _query_requests_food_stop(user_message):
        usable_cards = [
            card for card in usable_cards
            if _card_kind_for_plan_block(card) != "food"
            or _planner_food_card_allowed_for_requested_local_area(card, user_message)
        ]
    target_area = _extract_compact_plan_area_anchor(user_message)
    if target_area and _query_describes_single_area_plan(user_message):
...
_PLANNER_COUNT_UNIT_RE = re.compile(
    ...
    r"(?:(?:indoor|interior|inside|covered|cobert[oa]s?|fechad[oa]s?|"
    r"accessible|acess[ií]ve(?:l|is)|nearby|local|central|cheap|budget|"
    r"barat[oa]s?|gratuit[oa]s?|free|historic[oa]s?|historical|cultural|"
    r"small|large|pequen[oa]s?|grand(?:e|es)|calm|quiet|tranquil[oa]s?)\s+){0,4}"
...
def _score_food_card_for_meal_context(
    ...
    if preferred_area_key and _query_describes_single_area_plan(user_message):
        if _planner_food_card_allowed_for_requested_local_area(card, user_message):
            score += 220
        else:
            score -= 650

    if compact_short_plan and plan_has_central:
        if card_is_central:
            score += 150
...
def _planner_food_card_allowed_for_requested_local_area(
    card: Dict[str, str],
    user_message: str,
) -> bool:
    """Return whether a food card can satisfy a compact local-area meal request."""
    area_key, area_label, blockers = _planner_local_area_profile(user_message)
    if not area_key or not _query_describes_single_area_plan(user_message):
        return True
    basis = _normalize_planner_text(
        " ".join(
            str(card.get(key, "") or "")
            for key in ("name", "address", "venue", "description", "features", "distance", "url", "details_url")
        )
    )
    if not basis:
        return False
    if any(re.search(rf"\b{re.escape(blocker)}\b", basis) for blocker in blockers):
        return False

    area_terms = {
        "marvila": ("Marvila", "Beato"),
        "belem": ("Belém",),
        "alfama": ("Alfama",),
        "oriente": ("Oriente", "Parque das Nações", "Expo"),
        "marques_de_pombal": ("Marquês de Pombal", "Avenida da Liberdade", "Saldanha"),
        "central_corridor": ("Baixa", "Chiado", "Alfama", "Rossio", "Carmo"),
    }.get(area_key, (area_label,))
    return any(_planner_card_matches_area(card, term) for term in area_terms if term)
...
def _insert_requested_food_stop_if_needed(
    ...
    explicit_meal_label_requested = bool(
        re.search(
            r"\b(?:almoco|almoço|almocar|almoçar|lunch|jantar|dinner)\b",
            _normalize_planner_text(user_message),
        )
    )
    result = [
    ...
    existing_food_count = sum(1 for card in result if _card_kind_for_plan_block(card) == "food")
    ...
    for meal_kind in requested_meals:
        ...
        for candidate in available_candidates:
            ...
            if candidate_key and candidate_key not in used_food_keys:
                meal_card = _label_food_card_for_meal(
                    candidate,
                    meal_kind,
                    language,
                    len(requested_meals) > 1 or explicit_meal_label_requested,
                )
                used_food_keys.add(candidate_key)
                break
        if not meal_card:
            meal_card = _requested_custard_tart_evidence_card(user_message, language)
        if not meal_card:
...
def _requested_custard_tart_evidence_card(user_message: str, language: str) -> Dict[str, str]:
    """Return a grounded pastry card from local VisitLisboa data when available."""
    if not _query_requests_custard_tart_stop(user_message):
        return {}
    normalized_query = _normalize_planner_text(user_message)
    requested_area = _normalize_planner_text(_extract_requested_plan_area(user_message))
    if re.search(r"\bbel[eé]m|belem\b", normalized_query):
        requested_area = "belem"
    try:
        from tools.visitlisboa_api import _load_places_json
    except Exception:
        return {}
    ...
    for item in _load_places_json():
        raw_text = " ".join(
            str(item.get(key, ""))
            for key in ("title", "category", "short_description", "full_description", "location")
        )
        basis = _normalize_planner_text(raw_text)
        if not re.search(r"\b(?:pastel|pasteis|nata|custard|pastry|coffee shop|cafe)\b", basis):
            continue
        if requested_area and requested_area not in basis:
            continue
        category = str(item.get("category") or "").strip()
        contact = item.get("contact_info") if isinstance(item.get("contact_info"), dict) else {}
        links = item.get("information_links") if isinstance(item.get("information_links"), dict) else {}
        tripadvisor = item.get("tripadvisor") if isinstance(item.get("tripadvisor"), dict) else {}
        features = item.get("features") if isinstance(item.get("features"), list) else []
        website = str(contact.get("website") or links.get("website") or "").strip()
        email = str(contact.get("email") or "").strip()
        phone = str(contact.get("phone") or contact.get("telephone") or "").strip()
        rating = ""
        if tripadvisor.get("rating"):
            rating = str(tripadvisor.get("rating"))
            if tripadvisor.get("reviews_count"):
                rating += f"/5 ({tripadvisor.get('reviews_count')} avaliações)"
        candidates.append(
            {
                "name": str(item.get("title") or "").strip(),
                "category": category or ("Cafetaria" if language == "pt" else "Coffee shop"),
                "address": str(item.get("location") or "").strip(),
                "description": str(item.get("short_description") or "").strip(),
                "features": " · ".join(str(feature) for feature in features if feature),
                "rating": rating,
                "phone": phone,
                "email": email,
                "website_url": website,
                ...
                "url_label": "VisitLisboa",
                "details_url": str(item.get("url") or "").strip(),
    ...
    candidates.sort(
        key=lambda card: (
            2 if re.search(r"\bpasteis?\s+de\s+belem\b|\bpast[eé]is\s+de\s+bel[eé]m\b", _normalize_planner_text(card.get("name", ""))) else 0,
            1 if "belem" in _normalize_planner_text(card.get("address", "")) else 0,
            1 if card.get("rating") else 0,
        ),
        reverse=True,
    )
    return candidates[0]
...
def _query_treats_start_anchor_as_origin_only(user_message: str) -> bool:
    ...
    return bool(
        re.search(
            r"\b(?:comeco|comeca|comece|comecar|comecando|parto|partimos|saio|saimos|inicia|iniciar|iniciando|start|starting)\b",
            normalized_query,
        )
        and not re.search(
...
def _planner_dict_card_is_closed(card: Dict[str, str]) -> bool:
    ...
    return bool(_PLANNER_UNUSABLE_CLOSED_RE.search(basis))
...
def _requested_anchor_placeholder_cards(
    ...
    for label in requested_labels:
        ...
        placeholders.append(
            {
                "name": cleaned,
                "category": "Ponto do percurso pedido" if is_generic_route_anchor and is_pt
                else "Requested route point" if is_generic_route_anchor
                else "Paragem pedida" if is_pt
                else "Requested stop",
                "description": (
                    "Ponto indicado pelo utilizador; usei-o para respeitar o percurso, sem acrescentar detalhes oficiais não confirmados."
                    if is_generic_route_anchor and is_pt
                    else "User-provided route point; used to respect the requested path without adding unconfirmed official details."
...
def _replace_meal_blocks_with_contextual_evidence(
    ...
    food_candidates = [
        ...
        if _planner_food_card_allowed_for_requested_local_area(
            _evidence_card_to_planner_card(card),
            user_message,
        )
    ] or (
        [] if _planner_local_area_profile(user_message)[0] and _query_describes_single_area_plan(user_message)
        else food_candidates
    )
...
def _evidence_card_is_closed(card: EvidenceCard) -> bool:
    ...
    basis = _normalize_planner_text(
        " ".join(
            ...
            for value in (
                card.title,
                _evidence_field_value(card, "Description"),
                _evidence_field_value(card, "Hours"),
            )
        )
    )
    return bool(_PLANNER_UNUSABLE_CLOSED_RE.search(basis))
...
def _planner_response_missing_requested_plan_components(
    ...
    if _query_requests_food_stop(user_message) and not re.search(
        ...
        return True
    if re.search(r"\b(?:miradouro|miradouros|viewpoint|viewpoints|view\s+point|lookout|vista)\b", normalized_query) and not re.search(
        r"\b(?:miradouro|viewpoint|view\s+point|lookout|vista|panoram)\b",
        route_text,
    ):
        return True
    ...
    if _query_requests_cultural_stop(user_message) and not re.search(
        ...
        r"exposi[cç][aã]o|exhibition|galeria|gallery|ocean[aá]rio|aquarium|pavilh[aã]o\s+do\s+conhecimento|"
        r"miradouro|viewpoint|jardim|garden|parque|park)\b",
        route_text,
    ):
        return True
...
def _ensure_requested_component_placeholders_in_response(response: str, user_message: str, language: str) -> str:
    """Surface explicit requested route components that still disappeared."""
    if not response:
        return response or ""
    missing_food = _planner_response_missing_requested_food_stop(response, user_message)
    missing_components = _planner_response_missing_requested_plan_components(response, user_message)
    if not (missing_food or missing_components):
        return response or ""
    ...
    if not _query_requests_cafe_stop(user_message) and _query_requests_food_stop(user_message) and not re.search(
        ...
        meal_card = _requested_meal_placeholder_card(user_message, language)
        title = str(meal_card.get("name") or ("Almoço a confirmar" if is_pt else "Meal stop to confirm"))
        category = str(meal_card.get("category") or ("Refeição" if is_pt else "Meal stop"))
        limitation = (
            "O pedido inclui comida/refeição, mas o roteiro final ainda não tinha uma paragem alimentar verificável; confirma esta componente antes de fechar o plano."
            if is_pt
            else "The request includes food/a meal, but the final route still lacked a verifiable food stop; confirm this component before finalising the plan."
        )
        add_card("🍽️", title, category, limitation)
    ...
    if re.search(r"\b(?:miradouro|miradouros|viewpoint|viewpoints|view\s+point|lookout|vista)\b", normalized_query) and not re.search(
        ...
        add_card(
            "🏛️",
            f"Miradouro em {area}" if is_pt else f"Viewpoint in {area}",
            "Miradouro" if is_pt else "Viewpoint",
            (
                "O utilizador pediu explicitamente um miradouro, mas a selecção final não manteve um miradouro confirmado."
                if is_pt
                else "The user explicitly requested a viewpoint, but the final selection did not preserve a confirmed viewpoint."
            ),
        )
    ...
    if re.search(r"\b(?:jardim|jardins|garden|gardens|parque|parques|park|parks)\b", normalized_query) and not re.search(
        ...
        add_card(
            ...
            "Jardim / parque" if is_pt else "Garden / park",
            (
                "O utilizador pediu explicitamente um jardim/parque, mas a selecção final não manteve um resultado confirmado desse tipo."
                if is_pt
...
def _ensure_budget_food_limitation_in_response(response: str, user_message: str, language: str) -> str:
    """State when a requested low-cost meal is not confirmed by visible evidence."""
    if not response or not _query_requests_budget_food(user_message):
        return response or ""

    route_text = "\n".join(_planner_response_route_blocks(response))
    if not route_text:
        return response
    normalized_route = _normalize_planner_text(route_text)
    if not re.search(r"\b(?:restaurante|restaurant|almo[cç]o|almoco|lunch|jantar|dinner|refei[cç][aã]o|meal)\b|🍽️", normalized_route):
        return response

    route_ascii = unicodedata.normalize("NFKD", route_text)
    route_ascii = "".join(char for char in route_ascii if not unicodedata.combining(char)).lower()
    has_low_cost_evidence = bool(
        re.search(r"(?:<|menos\s+de|under|ate|até)\s*20\s*(?:€|eur|euros?)?", route_ascii)
        or re.search(r"\b(?:barat\w*|baixo\s+custo|low\s+cost|cheap|budget)\b", normalized_route)
    )
    if has_low_cost_evidence:
        return response
    # Idempotency guard: match the actual inserted note lead in BOTH languages so a
    # second pass (planner finalize + graph final-response apply this twice) is a no-op.
    # The EN note reads "you asked for a low-cost option ...", which does not contain the
    # literal "low-cost price was not confirmed", so the EN lead must be matched explicitly.
    if re.search(
        r"pre[cç]o\s+barato\s+n[aã]o\s+ficou\s+confirmado|"
        r"pediste\s+uma\s+op[cç][aã]o\s+barata|"
        r"you\s+asked\s+for\s+a\s+low-cost\s+option|"
        r"low-cost\s+price\s+was\s+not\s+confirmed",
        _normalize_planner_text(response),
    ):
        return response

    has_mid_or_high_price = bool(
        re.search(r"20\s*(?:€|eur|euros?)?\s*(?:a|to|-)\s*50", route_ascii)
        or re.search(r"(?:>|mais\s+de|over)\s*50\s*(?:€|eur|euros?)?", route_ascii)
    )
    if language == "pt":
        note = (
            "- **Preço:** pediste uma opção barata; nos dados visíveis, o preço barato não ficou confirmado"
            + (" e a paragem alimentar mostra uma faixa acima desse pedido." if has_mid_or_high_price else ".")
        )
        notes_heading = r"^###\s+⚠️\s+\*\*Notas finais\*\*"
    else:
        note = (
            "- **Price:** you asked for a low-cost option; the visible data does not confirm a low-cost price"
            + (" and the meal stop shows a higher price band." if has_mid_or_high_price else ".")
        )
        notes_heading = r"^###\s+⚠️\s+\*\*Final notes\*\*"
    ...
    for line in lines:
        output.append(line)
        if not inserted and re.match(notes_heading, line.strip(), flags=re.IGNORECASE):
            output.extend(["", note])
            inserted = True
    if not inserted:
        output = []
        for line in lines:
            if not inserted and _PLANNER_SOURCE_LINE_RE.match(line.strip()):
                output.extend([note, ""])
                inserted = True
            output.append(line)
        if not inserted:
...
def _planner_response_route_blocks(response: str) -> List[str]:
    ...
    for raw_line in str(response or "").splitlines():
        ...
        if re.search(r"\b(?:roteiro sugerido|suggested route)\b", normalized_line):
            in_route = True
            continue
        if in_route and not current and re.search(r"\b(?:resposta direta|direct answer)\b", normalized_line):
            continue
        if in_route and stripped == "---" and not current:
            continue
...
def _ensure_stop_by_stop_movement_in_response(
    ...
    if _query_requests_walking_only_plan(user_message) and not _query_requests_stop_by_stop_movement(user_message):
        return _enforce_walking_only_movement(cleaned, user_message, language)
    ...
    for (origin, origin_block), (destination, destination_block) in zip(title_blocks, title_blocks[1:5]):
        ...
        if wants_public_transport and not same_requested_area:
            ...
            ordered_bullets.append(
                (
                    f"- 🚶 **{origin} → {destination}:** deslocação a pé estimada — "
                    "não tenho dados exatos de percurso pedonal entre estes pontos; consulte a distância no mapa antes de partir."
                )
                if is_pt
                else (
                    f"- 🚶 **{origin} → {destination}:** estimated walking segment — "
                    "no exact pedestrian route data available for this leg; check the distance on a map before you go."
                )
            )
...
def _query_targets_future_or_multi_day_plan(user_message: str) -> bool:
    """Return whether today-specific labels should be neutralised in a plan."""
    normalized = _normalize_planner_text(user_message)
    if re.search(r"\b(?:hoje|today)\b", normalized):
        return False
    if (_extract_requested_day_count(user_message) or 0) >= 2:
        return True
    return bool(
        re.search(
            r"\b(?:amanha|tomorrow|depois\s+de\s+amanha|this\s+weekend|fim\s+de\s+semana|"
            r"este\s+sabado|este\s+domingo|sabado|domingo|next\s+(?:saturday|sunday|week|weekend))\b",
            normalized,
        )
    )


def _neutralize_today_hours_for_future_plan(
    response: str,
    user_message: str,
    language: str,
) -> str:
    """Avoid presenting source opening-hour labels as if a future plan is for today."""
    if not response or not _query_targets_future_or_multi_day_plan(user_message):
        return response
    if language == "pt":
        response = re.sub(
            r"(\*\*Hor[aá]rio:\*\*)\s*Hoje:\s*",
            r"\1 Horário disponível: ",
            response,
            flags=re.IGNORECASE,
        )
        response = re.sub(
            r"(\*\*Hor[aá]rio:\*\*)\s*Today:\s*",
            r"\1 Horário disponível: ",
            response,
            flags=re.IGNORECASE,
        )
...
def _ensure_planner_rhythm_guidance(
    response: str,
    user_message: str,
    language: str,
) -> str:
...
def _ensure_missing_origin_note_in_plan_response(
    response: str,
    user_message: str,
    language: str,
) -> str:
    """Disclose the missing starting point on itinerary answers without one.

    When a roteiro/itinerary request names no origin, the rendered plan starts
    at the first stop with no first movement leg. Instead of guessing an
    origin, append a single scoped note before the source footer telling the
    user where the plan starts and how to get the first leg added.

    Args:
        response: Final rendered planner answer in Markdown.
        user_message: Original user request used to detect an origin anchor.
        language: Response language ("pt" or "en").

    Returns:
        The response with at most one starting-point note added.
    """
    text = str(response or "")
    if not text.strip():
        return response
    user_key = _normalize_planner_text(user_message)
    if not re.search(
        r"\b(?:roteiro|itinerario|itinerary|plano|plan|visitar|visit|paragens|stops|miradouro|viewpoint|jantar|dinner|almoco|lunch)\b",
        user_key,
    ):
        return response
    has_plan_signal = bool(
        re.search(
            r"(?mi)^\s*(?:#{1,4}\s*)?(?:📍\s*)?(?:\*\*)?"
            r"(?:Roteiro sugerido|Suggested route|Suggested itinerary|Plano|Plan|Roteiro|Itinerary)",
            text,
        )
        or re.search(r"(?m)^\s*(?:[-*]\s*)?\*\*(?:🏷️|🏛️|📍|🍽️|☕|🥐)\s+[^*\n]{3,120}\*\*\s*$", text)
        or re.search(
            r"\b(?:Tempo sugerido|Suggested time|Hor[aá]rio sugerido|Suggested schedule|"
            r"Como te deslocas|How to move|Morada|Address|Mais detalhes|More details)\b",
            text,
            flags=re.IGNORECASE,
        )
    )
    if not has_plan_signal:
        return response
    if re.search(r"\*\*(?:Ponto de partida|Starting point):\*\*", text):
        return response
    if _extract_requested_plan_origin(user_message):
        return response
    user_text = str(user_message or "")
    if re.search(
        r"\b(?:daqui|aqui perto|de onde estou|onde estou|from here|my location|"
        r"minha localiza[cç][aã]o|current location)\b",
        user_text,
        flags=re.IGNORECASE,
    ) and not re.search(
        r"\b(?:n[aã]o\s+assumas?|sem\s+assumir|do\s+not\s+assume|don't\s+assume)\b",
        user_text,
        flags=re.IGNORECASE,
    ):
        return response
    if language == "pt":
        note = (
            "- ⚠️ **Ponto de partida:** não indicaste onde começas, por isso o "
            "roteiro arranca na primeira paragem sugerida. Diz-me o teu ponto de "
            "partida (hotel, estação ou bairro) para eu acrescentar esse primeiro trajeto."
        )
    else:
        note = (
            "- ⚠️ **Starting point:** you did not say where you start, so the "
            "route begins at the first suggested stop. Tell me your starting "
            "point (hotel, station, or neighbourhood) and I will add that first leg."
        )
    lines = text.splitlines()
    insert_at = len(lines)
    for index, line in enumerate(lines):
        if _PLANNER_SOURCE_LINE_RE.match(line.strip()):
            insert_at = index
            break
    block = [note, ""] if insert_at < len(lines) else ["", note]
    lines[insert_at:insert_at] = block
    return "\n".join(lines)


def _first_visible_plan_stop_name(response: str) -> str:
    """Return the first visible stop name from a rendered route section."""
    in_route_section = False
    field_labels = {
        "horario sugerido", "tempo sugerido", "categoria", "morada", "preco",
        "avaliacao", "telefone", "email", "website", "mais detalhes",
        "suggested time", "suggested duration", "category", "address", "price",
        "rating", "phone", "more details",
    }
    for raw_line in str(response or "").splitlines():
        stripped = raw_line.strip()
        stripped = re.sub(r"^#{1,6}\s+", "", stripped).strip()
        if re.search(r"\b(?:Roteiro sugerido|Suggested route|Suggested itinerary)\b", stripped, flags=re.IGNORECASE):
            in_route_section = True
            continue
        if not in_route_section:
            continue
        if re.search(r"\b(?:Como te deslocas|How to move)\b", stripped, flags=re.IGNORECASE):
            break
        if not stripped or stripped == "---":
            continue
        if re.search(r"\b(?:Resposta direta|Direct answer)\b", stripped, flags=re.IGNORECASE):
            continue
        match = re.match(
            r"^(?:[-*]\s*)?(?:🏷️|🏛️|📍|🍽️|☕|🥐)?\s*(?:\*\*)?(?P<name>[^*\n:][^*\n]{2,120}?)(?:\*\*)?\s*$",
            stripped,
        ...
        name = re.sub(r"^(?:🏷️|🏛️|📍|🍽️|☕|🥐)\s*", "", name).strip(" .:-")
        normalized = _normalize_planner_text(name)
        if not name or normalized in field_labels:
            continue
        if re.search(r"\b(?:horario|tempo|categoria|morada|preco|avaliacao|telefone|email|website|detalhes|"
                     r"time|duration|category|address|price|rating|phone|details)\b", normalized):
            continue
        return name
    return ""


def _ensure_requested_origin_first_leg_in_plan_response(
    response: str,
    user_message: str,
    language: str,
) -> str:
    """Ensure plans with an explicit origin show the first leg from that origin."""
    text = str(response or "")
    if not text.strip():
        return response
    origin = _extract_requested_plan_origin(user_message)
    if not origin:
        return response
    movement_text = _planner_movement_section_text(text) or text
    normalized_origin = _normalize_planner_text(origin)
    has_origin_route_line = any(
        normalized_origin in _normalize_planner_text(line)
        and ("→" in line or "->" in line)
        for line in movement_text.splitlines()
    )
    if normalized_origin and has_origin_route_line:
        return response
    first_stop = _first_visible_plan_stop_name(text)
    if not first_stop:
        return response
    if _normalize_planner_text(first_stop) == normalized_origin:
        return response
    if language == "pt":
        bullet = (
            f"- 🚶 **{origin} → {first_stop}:** começa em **{origin}**; "
            "a ligação exata para a primeira paragem não ficou confirmada nos dados recolhidos."
        )
        heading = "### 🚇 **Como te deslocas**"
    else:
        bullet = (
            f"- 🚶 **{origin} → {first_stop}:** start at **{origin}**; "
            "the exact first leg to the first stop was not confirmed in the gathered data."
        )
        heading = "### 🚇 **How to move**"
    if _normalize_planner_text(bullet) in _normalize_planner_text(text):
        return response

    section_re = re.compile(
        r"(?ms)(?P<header>^(?:###\s+)?[^\n]*(?:\*\*)?(?:Como te deslocas|How to move)(?:\*\*)?\s*(?:\n|$))"
        r"(?P<body>.*?)(?=(?:\n---\s*\n|\n(?:###\s+)?[^\n]*(?:Dicas|Tips|Notas finais|Final notes|"
        r"Adapta[cç][aã]o ao tempo|Weather adaptation)[^\n]*\n|\n📌\s+(?:\*\*)?(?:Fonte|Source):|\Z))"
    )

    def prepend_to_movement(match: re.Match[str]) -> str:
        body = match.group("body").strip("\n")
        if body:
            return f"{match.group('header')}{bullet}\n\n{body}\n"
        return f"{match.group('header')}{bullet}\n"

    if section_re.search(text):
        return section_re.sub(prepend_to_movement, text, count=1).strip()
...
def _planner_response_block_matches_requested_type(block: str, count_type: str) -> bool:
    ...
    type_block = "\n".join(
        line for line in str(block or "").splitlines()
        if not re.search(
            r"\b(?:website|bilhetes?|tickets?|email|e-mail|telefone|phone|morada|address|maps?)\b",
            line,
            flags=re.IGNORECASE,
        )
    )
    basis = _normalize_planner_text(type_block)
    basis = re.sub(r"https?://\S+|mailto:\S+|tel:\S+", " ", basis)
    basis = re.sub(r"\b[a-z0-9.-]+\.(?:pt|com|org|net|edu)\S*", " ", basis)
    food_or_meal_block = bool(
        re.search(
            r"\b(?:almo[cç]o|almoco|lunch|jantar|dinner|restaurante|restaurant|"
            r"categoria\s*:?\s*restaurante|category\s*:?\s*restaurant|"
            r"refeicao|refeição|meal)\b",
            basis,
        )
    )
    if count_type in {"museum", "monument", "viewpoint", "event"} and food_or_meal_block:
        return False
    ...
    basis_without_broad_category = re.sub(r"https?://\S+|mailto:\S+|tel:\S+", " ", basis_without_broad_category)
    basis_without_broad_category = re.sub(
        r"\b[a-z0-9.-]+\.(?:pt|com|org|net|edu)\S*",
        " ",
...
def _planner_route_block_ranges(response: str) -> List[tuple[int, int, str]]:
    ...
    for index, raw_line in enumerate(lines):
        ...
        if re.search(r"\b(?:roteiro sugerido|suggested route)\b", normalized_line):
            in_route = True
            continue
        if in_route and current_start is None and re.search(r"\b(?:resposta direta|direct answer)\b", normalized_line):
            continue
        if in_route and current_start is None and stripped == "---":
            continue
        if in_route and current_start is not None and stripped == "---":
            route_end = index
            break
        if in_route and stripped.startswith("### "):
            route_end = index
...
def _repair_response_requested_meal_time(response: str, user_message: str, language: str = "pt") -> str:
    """Add an explicit requested meal time to the visible meal block if missing.

    This is a deterministic repair for otherwise valid plans where the planner
    includes the requested lunch/dinner stop but drops the user-specified time.
    """
    if not response:
        return response or ""
    constraints = _requested_meal_time_constraints(user_message)
    if not constraints:
        return response
    lines = str(response).splitlines()
    changed = False
    ...
    for meal_label, time_label in constraints:
        if not time_label or time_label in "\n".join(lines):
            continue
        meal_key = _normalize_planner_text(meal_label)
        wants_lunch = bool(re.search(r"\b(?:almoco|lunch)\b", meal_key))
        wants_dinner = bool(re.search(r"\b(?:jantar|dinner)\b", meal_key))
        block_ranges = _planner_route_block_ranges("\n".join(lines))
        for start, _end, block in block_ranges:
            normalized_block = _normalize_planner_text(block)
            if wants_lunch and not re.search(r"\b(?:almoco|lunch|restaurante|restaurant|meal|refeicao)\b", normalized_block):
                continue
            if wants_dinner and not re.search(r"\b(?:jantar|dinner|restaurante|restaurant|meal|refeicao)\b", normalized_block):
                continue
            heading = lines[start]
            if time_label not in heading:
                lines[start] = re.sub(
                    r"^(\s*[-*]\s+\*\*(?:[\U0001F300-\U0001FAFF\u2600-\u27BF\uFE0F\u200D]+\s*)?)",
                    lambda match: f"{match.group(1)}{time_label} · ",
                    heading,
                    count=1,
                )
                detail = (
                    f"    - 📝 Hora pedida pelo utilizador: {time_label}"
                    if is_pt
                    else f"    - 📝 User-requested time: {time_label}"
                )
                lines.insert(start + 1, detail)
                changed = True
            break
    return re.sub(r"\n{3,}", "\n\n", "\n".join(lines)).strip() if changed else response
...
def _planner_response_has_closed_timed_stop(response: str, user_message: str) -> bool:
    ...
    for block in _planner_response_route_blocks(response):
        ...
        if has_requested_time and _PLANNER_UNUSABLE_CLOSED_RE.search(normalized_block):
            return True
    return False
...
def _planner_response_has_unusable_closed_stop(response: str, user_message: str) -> bool:
    ...
    for block in _planner_response_route_blocks(response):
        normalized_block = _normalize_planner_text(block)
        if not _PLANNER_UNUSABLE_CLOSED_RE.search(normalized_block):
            continue
        if re.search(
            r"\b(?:restaurante|restaurant|almoco|lunch|jantar|dinner|museu|museum|"
            r"monumento|monument|mosteiro|monastery|torre|tower|maat|atracao|attraction|"
            r"local|place|visita|visit)\b",
            normalized_block,
        ):
            return True
    return False
...
def _planner_response_has_generic_component_placeholder(response: str) -> bool:
    """Return whether the visible plan exposes a generic missing-component slot."""
    normalized = _normalize_planner_text(response)
    return bool(
        re.search(
            r"\b(?:miradouro\s+em\s+lisboa|viewpoint\s+in\s+lisbon|"
            r"jardim\s+em\s+lisboa|garden\s+in\s+lisbon|"
            r"paragem\s+cultural\s+em\s+lisboa|cultural\s+stop\s+in\s+lisbon|"
            r"selec[c]?ao\s+final\s+nao\s+manteve|final\s+selection\s+did\s+not\s+preserve)\b",
            normalized,
        )
    )


def _planner_response_has_duplicate_route_titles(response: str) -> bool:
    """Return whether the suggested route repeats the same visible stop title."""
    seen: set[str] = set()
    for block in _planner_response_route_blocks(response):
        title = _normalize_planner_text(_planner_response_block_title(block))
        title = re.sub(r"^(?:almoco|lunch|jantar|dinner)\s+", "", title).strip()
        if not title or title in {"lisboa", "lisbon"}:
            continue
        if title in seen:
            return True
        seen.add(title)
...
def _planner_response_missing_requested_time_constraints(response: str, user_message: str) -> bool:
    ...
    anchor_constraints = _requested_anchor_time_constraints(user_message)
    meal_constraints = _requested_meal_time_constraints(user_message)
    all_constraints = anchor_constraints + meal_constraints
    if not all_constraints:
        return False

    for meal_label, time_label in meal_constraints:
        if time_label and time_label not in response:
            return True
    ...
    if not blocks:
        return True
    for label, time_label in anchor_constraints:
        normalized_label = _normalize_planner_text(label)
...
def _planner_response_violates_explicit_preference_contract(response: str, user_message: str) -> bool:
    ...
    return bool(
        ...
        or _planner_response_has_generic_component_placeholder(response)
        or _planner_response_has_duplicate_route_titles(response)
...
class PlannerAgent(BaseAgent):
    ...
    def invoke(
        ...
        final_response = _neutralize_today_hours_for_future_plan(final_response, user_message, language)
        final_response = _ensure_planner_rhythm_guidance(final_response, user_message, language)
        final_response = _ensure_requested_component_placeholders_in_response(final_response, user_message, language)
        final_response = _ensure_budget_food_limitation_in_response(final_response, user_message, language)
