...
if TYPE_CHECKING:
    ...
    from agent.planning.legs import LegRequest

from agent.agents.base import BaseAgent
from agent.planning.legs import departure_times, door_to_door_minutes, localize_departure_notes
...
def _strip_endpoint_mode_clauses(value: str) -> str:
    ...
    cleanup_patterns = (
        ...
        rf"\s+(?:sem|without|no|not\s+by)\s+"
        rf"(?:(?:ser|usar|apanhar|tomar|recorrer|ir)\s+(?:de\s+|a\s+)?)?"
        rf"(?:(?:com|por|by)\s+)?(?:o\s+|a\s+|the\s+)?(?:{mode_terms})\b.*$",
        ...
        r"\s+(?:como\s+(?:posso|devo|é\s+que\s+posso)\s+(?:fazer|ir|chegar)|"
        r"how\s+(?:can|do)\s+i\s+(?:do|go|get|travel))\b.*$",
...
def _clean_query_fragment(part: str) -> str:
    ...
    # Strip a trailing stated departure/arrival time (with any leading day or
    # daypart word) so it does not pollute the geocoded endpoint. Route tools in
    # this runtime resolve "now"; a requested clock time is handled elsewhere and
    # must not leak into the place name. Otherwise "Cristo Rei as 18:00" geocodes
    # to an unrelated homonym instead of the Cristo Rei monument.
    part = re.sub(
        r"\s+(?:(?:amanh[ãa]|hoje|logo|tonight|tomorrow|de\s+manh[ãa]|"
        r"[àa]\s+(?:tarde|noite)|this\s+(?:morning|afternoon|evening|weekend))\s+)?"
    ...
    # Strip a trailing standalone day/daypart word with no explicit clock time
    # (e.g. "Belem amanha", "Belem tonight").
    part = re.sub(
        r"\s+(?:amanh[ãa]|hoje|logo|tonight|tomorrow|de\s+manh[ãa]|"
        r"[àa]\s+(?:tarde|noite)|this\s+(?:morning|afternoon|evening|weekend))\s*$",
        "",
    ...
    # Route patterns like "do Saldanha para a Amadora" can be partially
    # captured by a broader "do X a Y" parser, leaving a dangling connector in
    # the origin ("Saldanha para"). Drop terminal connectors before resolution.
    part = re.sub(r"\b(at[eé]|ate|para|to)\s*$", "", part, flags=re.IGNORECASE)
    part = re.sub(
...
def _extract_route_code(query: str) -> Optional[str]:
    ...
    patterns = [
        ...
        r"\b(?:o|a|o\s+n[oº]|n[oº])\s+(?P<route>\d{2,4})\s+(?:do|da|de|no|na|para|from|to|between|entre|at|em|a partir)\b",
        r"\b(?:do|da|no|na|the|for)\s+(?P<route>\d{2,4}[A-Za-z]?)\b",
        r"\b(?:pr[oó]xim[oa]?|next)\s+(?P<route>\d{2,4}[A-Za-z]?)\b",
        r"\b(?:o|a)\s+(?P<route>\d{3,4})\b",
    ]
...
def _build_carris_urban_tool_spec(user_message: str) -> Optional[Dict[str, Any]]:
    ...
    if route_code and re.search(
        r"\b(real-time|realtime|live|gps|location|locations|position|positions|where\s+is|agora|now)\b"
        r"|onde\s+est[aá]",
        query_lower,
    ):
        return {
    ...
    if route_code and re.search(
        r"\b(funciona|funcionar|circula|circular|a\s+funcionar|running|run|service|status)\b",
        query_lower,
    ):
        return {
            "name": "carris_get_realtime_vehicles",
    ...
    if route_code and re.search(
        r"\b(passa\s+por\s+onde|por\s+onde|percurso|trajeto|trajecto|paragens?|stops?|route|rota)\b",
        query_lower,
    ):
    ...
    if endpoints and re.search(
        ...
        return {
            ...
            "args": _carris_route_tool_args(endpoints[0], endpoints[1], query),
        }

    if route_code and endpoints and not re.search(
        r"\b(cp|comboio|comboios|train|trains|metro|linha\s+(?:vermelha|verde|azul|amarela)|"
        r"red\s+line|blue\s+line|green\s+line|yellow\s+line)\b",
        query_lower,
    ):
        return {
            "name": "carris_find_routes_between",
    ...
    *,
    ...
    def _valid_endpoint(value: str) -> bool:
        ...
        if normalized in {
            ...
            return False
        if re.fullmatch(
            r"(?:metro|autocarros?|bus(?:es)?|comboios?|train(?:s)?)\s+(?:para|ate|a|to)",
            normalized,
        ):
            return False
...
def _parse_route_mode_preferences(user_message: str) -> Dict[str, bool]:
    ...
    exclude_bus_phrases = [
        ...
        "sem usar autocarro",
        "sem usar autocarros",
        "sem apanhar autocarro",
        "sem apanhar autocarros",
        "nao quero autocarro",
        "não quero autocarro",
    ...
    optional_use = r"(?:\s+(?:ser|usar|apanhar|tomar|recorrer\s+a|using|taking|take|catch))?"
    ...
    if not alternative_mode_request:
        ...
        bus_only = bus_only or bool(
            re.search(
                r"\b(?:pr[oó]xim(?:o|a|os|as)|next|partidas?|chegadas?|departures?|arrivals?)\s+"
                r"(?:dos?\s+|das?\s+|de\s+|the\s+)?(?:bus|buses|autocarro|autocarros)\b",
                normalized,
            )
            or re.search(
                r"\b(?:bus|buses|autocarro|autocarros|carris)\s+"
                r"(?:dos|das|do|da|de|from)\b",
                normalized,
            )
        )
        tram_only = tram_only or bool(
        ...
        # "Que metro apanho?", "which metro should I take?", "take the metro":
        # the user asked for the Metro, as "que autocarro" asks for the bus.
        metro_only = metro_only or bool(
            re.search(
                r"\b(?:que|qual|which|what)\s+(?:linha\s+(?:de|do)\s+)?metro\s+"
                r"(?:(?:devo|deveria|posso|should(?:\s+i)?|can(?:\s+i)?|do\s+i)\s+)?"
                r"(?:apanhar|apanho|tomar|usar|take|catch|use)\b|\b"
                r"(?:apanhar|apanho|tomar|usar|take|catch|use)\s+(?:o\s+|a\s+|the\s+)?metro\b",
                normalized,
            )
        )
        train_only = train_only or bool(
...
def _query_prioritizes_less_walking(user_message: str) -> bool:
    """Return whether the route request explicitly prioritizes less walking."""
    normalized = _normalize_token(user_message)
    return bool(
        re.search(
            r"\b(?:menos|pouca|menor)\s+caminhada\b|"
            r"\b(?:caminhar\s+pouco|andar\s+pouco)\b|"
            r"\b(?:least|less|minimal)\s+walking\b",
            normalized,
            flags=re.IGNORECASE,
        )
    )
...
def _requested_route_option_modes(user_message: str) -> set[str]:
    ...
    if re.search(r"\bmetro\b", normalized):
        modes.add("metro")
    if re.search(r"\b(?:bus|buses|autocarro|autocarros|carris|carris\s+metropolitana)\b", normalized):
        modes.add("bus")
...
_ANOTHER_DAY_PHRASE_RE = re.compile(
    r"\b(?:depois de amanha|day after tomorrow|amanha|tomorrow|(?:este |this |no |next )?fim de semana|(?:this |next )?weekend|"
    r"segunda-feira|terca-feira|quarta-feira|quinta-feira|sexta-feira|(?:no |na |ao |next |on )?(?:sabado|domingo)|"
    r"monday|tuesday|wednesday|thursday|friday|saturday|sunday|\d{1,2}[/-]\d{1,2}(?:[/-]\d{2,4})?)\b"
)
# Lines that only hold for leaving now: next departures, live positions, waits.
_LIVE_DEPARTURE_LINE_RE = re.compile(
    r"^[ \t]*(?:[-*][ \t]+)?(?:[^\w\s*]+[ \t]*)?(?:\*\*)?(?:Pr[oó]ximas partidas|Next departures|Tempo real|Real[- ]time|"
    r"Pr[oó]ximo metro|Next metro|Espera(?: estimada)?|Waiting time|Wait)(?:\*\*)?\s*:.*$\n?"
    r"|^[ \t]*📡.*$\n?",
    re.IGNORECASE | re.MULTILINE,
)


def _drop_live_departures_for_another_day(response: str, user_message: str, language: str) -> str:
    """Remove live departures from an answer about a trip on another day.

    Next departures and waiting times describe leaving now; for "tomorrow" or
    "on Saturday" they would read as that day's timetable. The route itself
    (lines, stops, transfers, in-vehicle time) stays.

    Args:
        response: Finalized transport answer.
        user_message: The user's question.
        language: Output language.

    Returns:
        The answer without live-departure lines, plus one note, when the trip
        is for another day; otherwise the answer unchanged.
    """
    from agent.planning.brief import PlanBrief, plan_date
    from tools.utils import lisbon_now

    match = _ANOTHER_DAY_PHRASE_RE.search(_normalize_token(user_message))
    if not match or not response:
        return response
    today = lisbon_now().date()
    if plan_date(PlanBrief(day=match.group(0)), today) == today:
        return response
    cleaned = _LIVE_DEPARTURE_LINE_RE.sub("", response)
    if cleaned == response:
        return response
    note = (
        "ℹ️ As partidas em tempo real só valem para sair agora; para o dia da viagem, confirma os horários no operador."
        if language == "pt"
        else "ℹ️ Live departures only apply to leaving now; for the day of the trip, check the timetable with the operator."
    )
    source = re.search(r"(?m)^📌\s+\*\*(?:Fonte|Source):\*\*.*$", cleaned)
    if source:
        return f"{cleaned[: source.start()].rstrip()}\n\n{note}\n\n{cleaned[source.start():]}".strip()
    return f"{cleaned.rstrip()}\n\n{note}"


def _best_carris_option_sentence(response: str, language: str, user_message: str = "") -> str:
    """Summarize the fastest Carris option listed in an answer, or return "" (see ``carris_option_sentence``)."""
    from agent.planning.legs import carris_option_sentence, extract_carris_options

    return carris_option_sentence(extract_carris_options(response), language, user_message)


def _ensure_transport_direct_answer_block(
    response: str,
    user_message: str,
    language: str,
) -> str:
    """Ensure transport route answers keep a direct-answer block before details."""
    if not response or re.search(r"\*\*(?:Resposta direta|Direct answer):\*\*", response, flags=re.IGNORECASE):
        return response
    ...
    if "→" not in heading and not re.search(r"\b(?:rota|route|trajeto|trip)\b", heading, flags=re.IGNORECASE):
        return response

    normalized_query = _normalize_token(user_message)
    concrete = _best_carris_option_sentence(response, language, user_message)
    if concrete:
        direct = f"✅ **{'Resposta direta' if language == 'pt' else 'Direct answer'}:** {concrete}"
    elif language == "pt":
        if re.search(r"\b(?:menos|pouca|menor)\s+caminhada\b", normalized_query):
            direct = (
                "✅ **Resposta direta:** encontrei uma opção de transporte suportada que reduz a caminhada "
                "face a uma deslocação totalmente a pé; confirma os detalhes antes de sair."
            )
        else:
            direct = "✅ **Resposta direta:** encontrei uma opção de transporte suportada para este trajeto."
    else:
        if re.search(r"\b(?:less|least|minimal)\s+walking\b", normalized_query):
            direct = (
                "✅ **Direct answer:** I found a supported transport option that reduces walking "
                "compared with doing the whole trip on foot; confirm the details before leaving."
            )
        else:
            direct = "✅ **Direct answer:** I found a supported transport option for this trip."
    ...
    if after and after[0].strip() != "---":
        insertion.extend(["", "---"])
    insertion.append("")
    return "\n".join(before + insertion + after).strip()
...
def _tool_result_indicates_no_match(result: str) -> bool:
    ...
    negative_markers = [
        ...
        "sem rota direta de autocarro confirmada",
        "no direct buses",
...
def _extract_requested_departure_clock(user_message: str) -> str:
    """Extract a same-day requested departure time from route wording.

    Returns:
        A normalized ``HH:MM`` string when the user gives an explicit clock time
        (for example ``às 9h``, ``às 18:00`` or ``at 3pm``), otherwise ``""``.
    """
    text = str(user_message or "")
    if not text:
        return ""
    match = re.search(
        r"\b(?:[àaá]s|pelas|por\s+volta\s+das|cerca\s+das|at|around)\s+"
        r"(?P<hour>\d{1,2})(?:\s*[:hH]\s*(?P<minute>\d{2}))?\s*"
        r"(?P<suffix>a\.?m\.?|p\.?m\.?|am|pm|h|horas?)?\b",
        text,
        flags=re.IGNORECASE,
    )
    if not match:
    ...
    suffix = re.sub(r"\.", "", (match.group("suffix") or "").lower())
...
def _carris_route_tool_args(
    origin: str,
    destination: str,
    user_message: str = "",
    search_radius_km: float = 0.8,
) -> Dict[str, Any]:
    """Build Carris route tool arguments, preserving an explicit departure time."""
    args: Dict[str, Any] = {
        "origin": origin,
        "destination": destination,
        "search_radius_km": search_radius_km,
    }
    requested_clock = _extract_requested_departure_clock(user_message)
    if requested_clock:
        args["start_time"] = requested_clock
    return args
...
def _format_carris_route_detail(
    ...
    if next_match:
        ...
        if language != "pt":
            times_text = localize_departure_notes(times_text, language)
        lines: List[str] = []
        if times_text:
            if language == "pt":
    ...
    initial_walk_match = re.match(
        r"^(?:Initial walk|Caminhada inicial):\s*~\s*(?P<minutes>\d+)\s*min\s+(?:to|até(?:\s+à|\s+ao)?)\s+(?P<stop>.+?)\.?$",
        stripped,
        flags=re.IGNORECASE,
    )
    if initial_walk_match:
        minutes = initial_walk_match.group("minutes")
        stop = initial_walk_match.group("stop").strip()
        if language == "pt":
            return [f"    - 🚶 **Caminhada inicial:** ~{minutes} min até **{stop}**."]
        return [f"    - 🚶 **Initial walk:** ~{minutes} min to **{stop}**."]
    ...
    if re.search(
        ...
        r"n[ãa]o foram confirmadas pr[óo]ximas partidas hoje na paragem de origem",
        stripped,
        flags=re.IGNORECASE,
    ):
        if language == "pt":
    ...
    if travel_match:
        ...
        if language == "pt":
            return [f"    - ⏱️ **Tempo estimado:** {travel_text}"]
        return [f"    - ⏱️ **Estimated travel time:** {travel_text}"]
...
def _format_carris_mode_section_markdown(
    ...
    def walk_minutes(entry: Dict[str, Any], which: str) -> Optional[int]:
        """Return the initial or final walk of a route entry, in minutes."""
        for detail in entry.get("details", []):
            match = re.match(
                rf"^(?:{which})\s*:\s*~?\s*(\d+)\s*min",
                str(detail).strip(),
                flags=re.IGNORECASE,
            )
            if match:
    ...
    def route_sort_key(entry: Dict[str, Any]) -> tuple[int, int, int, str]:
        """Rank by the time a traveller spends: walk, wait, ride, and walk on.

        Options with confirmed departures come first; among them a closer
        stop with a sooner departure beats a shorter ride from a farther stop.
        """
        departures: List[str] = []
        for detail in entry.get("details", []):
            departures = departures or departure_times(str(detail))
        total = door_to_door_minutes(
            walk_minutes(entry, "Caminhada inicial|Initial walk"),
            travel_minutes(entry),
            walk_minutes(entry, "Caminhada final|Final walk"),
            departures,
        )
        return (
    ...
    for entry in ordered_entries:
        ...
        if language != "pt":
            # The tool writes the headsign in Portuguese ("para Moscavide").
            summary = re.sub(r"^para\s+", "to ", summary, flags=re.IGNORECASE)
        block_lines = [f"- **{entry['route']}**: {summary}"]
        for detail in entry.get("details", []):
            block_lines.extend(
        ...
        # "Paragem: X" repeats the boarding stop already named under "Paragens".
        board = re.search(r"\*\*(?:Paragens|Stops):\*\*\s*(?:apanha em|board at)\s+\*\*(?P<stop>[^*]+)\*\*", "\n".join(block_lines))
        if board:
            block_lines = [
                line for line in block_lines
                if not re.match(rf"^\s*-\s+(?:🚏\s+)?\*\*(?:Paragem|Stop):\*\*\s*{re.escape(board.group('stop').strip())}\s*$", line)
            ]
        blocks.append("\n".join(block_lines))
    ...
    if omitted_count:
        if language == "pt":
            ...
            verb = "foi omitida" if omitted_count == 1 else "foram omitidas"
            blocks.append(f"- ℹ️ Mais {omitted_count} {noun} com menor evidência imediata {verb} para manter a resposta acionável.")
        else:
...
def _summarize_recommended_carris_option(
    markdown: str,
    language: str,
    scheduled_from: str = "",
) -> str:
    ...
    # "Real time: upcoming departures confirmed" only when times were listed,
    # never under "no departures were confirmed today".
    has_live_departures = bool(
        departures_match and re.search(r"\d{1,2}:\d{2}|\d+\s*min", departures_match.group("departures"))
    )
    ...
    if language == "pt":
        ...
        if scheduled_from:
            summary += (
                f"\n- 🕒 **Hora pedida:** partidas programadas a partir de **{scheduled_from}**; "
                "não trato estes horários como partidas em tempo real."
            )
        elif has_live_departures:
    ...
    if scheduled_from:
        summary += (
            f"\n- 🕒 **Requested time:** scheduled departures from **{scheduled_from}**; "
            "these are not being presented as real-time departures."
        )
    elif has_live_departures:
...
def _metropolitana_direct_sentence(block: str, language: str) -> str:
    """Summarize the first direct Carris Metropolitana option of a block, or return "".

    "Option 1 · Board at: Cacilhas (Terminal) P5 · Alight at: ALMADA (AV CRISTO
    REI N41) · Lines: 3001 → Almada (Cristo Rei)" becomes one sentence.
    """
    text = str(block or "")
    board = re.search(r"\*\*(?:Board at|Embarque em|Apanhar em|Apanha em|Embarcar em):\*\*\s*([^|\n]+)", text)
    alight = re.search(r"\*\*(?:Alight at|Sair em|Sai em|Saída em|Desembarque em):\*\*\s*([^|\n]+)", text)
    lines = re.search(r"\*\*(?:Lines?|Linhas?):\*\*\s*([^\n]+)", text)
    if not (board and alight and lines):
        return ""
    line_text = lines.group(1).strip()
    first_line = re.match(r"(\d{4}[A-Z]?)", line_text)
    line_label = first_line.group(1) if first_line else line_text
    if language == "pt":
        return (
            f"Sim, há ligação direta: apanha o **{line_label}** da Carris Metropolitana em **{board.group(1).strip()}** "
            f"e sai em **{alight.group(1).strip()}**."
        )
    return (
        f"Yes, there is a direct bus: take Carris Metropolitana **{line_label}** at **{board.group(1).strip()}** "
        f"and get off at **{alight.group(1).strip()}**."
    )


def _direct_line_from_recommended_option(recommended_option: str, language: str) -> tuple[str, str]:
    """Turn the "best confirmed option" summary into the direct answer.

    Args:
        recommended_option: Output of ``_summarize_recommended_carris_option``.
        language: Output language.

    Returns:
        ``(direct_line, remaining_lines)``: the direct answer naming the line,
        stops, time, and next departure, and the summary lines left to show.
    """
    first_line, _, rest = recommended_option.partition("\n")
    best = re.sub(
        r"^\s*-\s*✅\s*\*\*(?:Melhor opção confirmada|Best confirmed option):\*\*\s*",
        "",
        first_line,
    ).strip().rstrip(".")
    is_pt = language == "pt"
    label = "Resposta direta" if is_pt else "Direct answer"
    if not best or best == first_line.strip():
        return f"✅ **{label}:** {first_line.strip()}", rest.strip()
    sentence = best[0].upper() + best[1:]
    departures = re.search(r"\*\*(?:Próximas partidas|Next departures):\*\*\s*([^\n]+)", rest)
    if departures:
        first_departure = departures.group(1).split(",")[0].strip()
        # Only a clock time: "no departures confirmed today" is not a time.
        if re.match(r"\d{1,2}:\d{2}\b", first_departure):
            sentence += f"; o próximo sai às {first_departure}" if is_pt else f"; the next one leaves at {first_departure}"
    return f"✅ **{label}:** {sentence.rstrip('.')}.", rest.strip()
...
def _build_mode_filtered_carris_route_response(
    ...
    requested_clock = _extract_requested_departure_clock(user_message)
    recommended = _summarize_recommended_carris_option(
        selected_markdown,
        language,
        scheduled_from=requested_clock,
    )
    # The direct answer names the lines listed below it.
    option_lines = list(
        dict.fromkeys(match.strip() for match in re.findall(r"(?m)^-\s+\*\*([^*\n]{1,12})\*\*:", selected_markdown))
    )
    if language == "pt":
        mode_label = "Elétrico" if is_tram else "Autocarro"
        origin_display = _get_transport_display_name(origin)
        destination_display = _get_transport_display_name(destination)
        title = f"### {'🚋' if is_tram else '🚌'} **{origin_display} → {destination_display}**"
        ...
        if asks_other_same_mode and option_count <= 1:
            ...
            direct = (
                f"✅ **Resposta direta:** para este trajeto há {'as linhas' if len(option_lines) > 1 else 'a linha'} de "
                f"{mode_label.lower()} {_join_bold(option_lines, 'pt')}."
            )
        else:
            direct = (
        ...
        if asks_other_same_mode and option_count <= 1:
            ...
            direct = (
                f"✅ **Direct answer:** for this trip you can take {mode_label.lower()} "
                f"{_join_bold(option_lines, 'en')}."
            )
        else:
            direct = (
    ...
    if requested_clock:
        parts.extend([
            "",
            (
                f"🕒 **Hora pedida:** mostro partidas programadas a partir de **{requested_clock}**."
                if language == "pt"
                else f"🕒 **Requested time:** showing scheduled departures from **{requested_clock}**."
            ),
        ])
...
def _join_bold(items: List[str], language: str) -> str:
    """Join names in bold as prose ("**708**, **744** e **759**")."""
    bold = [f"**{item}**" for item in items]
    if len(bold) <= 1:
        return "".join(bold)
    return ", ".join(bold[:-1]) + (" e " if language == "pt" else " and ") + bold[-1]
...
def _build_carris_surface_route_response(
    ...
    alternative_requested = bool(
        re.search(
            r"\b(?:alternativas?|op[cç][oõ]es|outra\s+op[cç][aã]o|diferente|"
            r"different|another\s+option|other\s+option|options)\b",
            user_message,
            flags=re.IGNORECASE,
        )
    )
    option_count = _count_formatted_carris_options("\n".join(part for part in (tram_markdown, bus_markdown) if part))
    if alternative_requested and option_count <= 1:
        direct = (
            f"⚠️ **Resposta direta:** só confirmei **uma opção direta** na Carris Urban entre **{origin_display}** e **{destination_display}**; "
            "não encontrei uma alternativa diferente suportada nos dados disponíveis."
            if is_pt
            else f"⚠️ **Direct answer:** I confirmed only **one direct option** on Carris Urban between **{origin_display}** and **{destination_display}**; "
            "I did not find a different supported alternative in the available data."
        )
    else:
        direct = (
            f"✅ **Resposta direta:** encontrei uma opção direta na Carris Urban entre **{origin_display}** e **{destination_display}**."
            if is_pt and not alternative_requested
            else f"✅ **Resposta direta:** encontrei opções de superfície na Carris Urban entre **{origin_display}** e **{destination_display}**."
            if is_pt
            else f"✅ **Direct answer:** I found a direct Carris Urban option between **{origin_display}** and **{destination_display}**."
            if not alternative_requested
            else f"✅ **Direct answer:** I found Carris Urban surface options between **{origin_display}** and **{destination_display}**."
        )
    source = (
    ...
    requested_route_code = _extract_route_code(user_message)
    visible_surface_options = "\n".join(part for part in (tram_markdown, bus_markdown) if part)
    if requested_route_code and not re.search(
        rf"\b(?:Linha|Line)\s+{re.escape(requested_route_code)}\b|\b{re.escape(requested_route_code)}\b",
        visible_surface_options,
        flags=re.IGNORECASE,
    ):
        parts.extend([
            "",
            (
                f"⚠️ **Nota sobre a linha {requested_route_code}:** não encontrei a **{requested_route_code}** "
                ...
                if is_pt
                else f"⚠️ **Note on line {requested_route_code}:** I could not confirm **{requested_route_code}** "
                "for this origin/destination in the available Carris data, so I show confirmed Carris alternatives."
            ),
        ])
    requested_clock = _extract_requested_departure_clock(user_message)
    recommended = _summarize_recommended_carris_option(
        bus_markdown or tram_markdown,
        language,
        scheduled_from=requested_clock,
    )
    if recommended and not alternative_requested:
        parts.extend(["", recommended])
    parts.extend(["", "---", ""])
    if tram_markdown:
        parts.extend([
...
def _localize_metropolitana_direct_bus_block(text: str, language: str) -> str:
    ...
    # Localize the pattern-based direction notes emitted by find_bus_routes so
    # they do not leak English into a Portuguese answer.
    localized = re.sub(
        r"\*\*Confirmed direction:\*\*\s*origin stop appears before destination "
        r"stop in the official pattern\s*\((?P<stops>\d+)\s*stops on-board\)\.",
        lambda match: (
            "**Direção confirmada:** a paragem de origem surge antes da de destino "
            f"no percurso oficial ({match.group('stops')} paragens a bordo)."
        ),
        localized,
        flags=re.IGNORECASE,
    )
    localized = re.sub(
        r"\*\*Direction:\*\*\s*line overlap found, but the official stop order "
        r"could not be confirmed from patterns\.",
        "**Direção:** há sobreposição de linha, mas não foi possível confirmar a "
        "ordem oficial das paragens.",
        localized,
        flags=re.IGNORECASE,
    )
...
def _infer_language(user_message: str, context: str) -> str:
    ...
    return resolved_language or infer_response_language(user_query=user_message, default="en")
...
def _is_future_transport_planning_query(user_message: str) -> bool:
    ...
    if not normalized:
        return False
    if _extract_requested_departure_clock(user_message):
        return True
...
def _build_unsupported_transport_scope_response(
    ...
    # The networks LISBOA reads, linked, so the user can check them directly.
    supported_links = (
        "[Metro de Lisboa](https://www.metrolisboa.pt), [Carris](https://www.carris.pt), "
        "[Carris Metropolitana](https://www.carrismetropolitana.pt) e [CP](https://www.cp.pt)"
    )
    operator_links = ", ".join(
        link.replace("[*", "[").replace("*]", "]") for link in dict.fromkeys(source_tokens)
    )
    scope_icon = "⛴️" if unsupported_modes == ["ferries"] else "🧭"
    if language == "pt":
        if not is_long_distance_only:
            return "\n".join(
                [
                    f"### {scope_icon} **Rede fora do âmbito confirmado**",
                    "",
                    f"✅ **Resposta direta:** não consigo confirmar {unsupported_label} em tempo real neste sistema.",
                    "",
                    "---",
                    "",
                    f"- 🔗 **Horários deste operador:** {operator_links}" if operator_links else "",
                    f"- ✅ **Redes que o LISBOA consulta:** {supported_links}.",
                    "- 💡 **Alternativa:** pergunta-me a mesma viagem de Metro, autocarro ou comboio e procuro a melhor ligação suportada.",
                ]
            ).strip()
        return "\n".join(
            [
    ...
    if not is_long_distance_only:
        supported_links_en = supported_links.replace(" e [CP]", " and [CP]")
        lines: list[str] = [
            f"### {scope_icon} **Network outside confirmed scope**",
            "",
            f"✅ **Direct answer:** I can't verify {unsupported_label} in real time in this system.",
            "",
            "---",
            "",
            *([f"- 🔗 **Operator timetables:** {operator_links}"] if operator_links else []),
            f"- ✅ **Networks LISBOA checks:** {supported_links_en}.",
            "- 💡 **Alternative:** ask me for the same trip by Metro, bus, or train and I will look for the best supported connection.",
        ]
...
def _get_transport_display_name(location: Optional[str], detailed: bool = False) -> str:
    ...
    from tools.metrolisboa_api import LISBON_LANDMARKS, get_landmark_info
    ...
    # Only the curated landmarks rename a place (NOVA IMS branding); a place the
    # geocoder found by name may be a namesake ("Jardins do Cristo Rei" in
    # Lisbon for Almada's Cristo Rei), so the user's words stay.
    if landmark and any(landmark is info for info in LISBON_LANDMARKS.values()):
        if detailed:
            return str(
...
def _localize_metro_status_text(
    ...
    *,
    ...
    if raw_interrupted_match:
        ...
        # "... e Cidade Universitária devido a avaria na sinalização": the
        # cause follows the station name and is not part of it.
        parts = re.split(r"\s+(?=(?:devido|due\s+to|por\s+motivo)\b)", end, maxsplit=1)
        end = parts[0].strip(" .")
        cause = parts[1].strip(" .") if len(parts) > 1 else ""
        if language == "pt":
            return f"circulação interrompida entre **{start}** e **{end}**" + (f" {cause}" if cause else "")
        for pattern, replacement in _METRO_STATUS_PHRASES_EN:
            cause = re.sub(pattern, replacement, cause, flags=re.IGNORECASE)
        return f"service is interrupted between **{start}** and **{end}**" + (f" {cause}" if cause else "")
    ...
    if language == "en":
        # The operator writes statuses in Portuguese; translate the usual phrases.
        translated = raw_status
        for pattern, replacement in _METRO_STATUS_PHRASES_EN:
...
_METRO_STATUS_PHRASES_EN = (
    (r"circula[cç][aã]o\s+interrompida", "service interrupted"),
    (r"circula[cç][aã]o\s+com\s+perturba[cç][oõ]es", "service disrupted"),
    (r"circula[cç][aã]o\s+condicionada", "restricted service"),
    (r"devido\s+a\s+avaria\s+na\s+sinaliza[cç][aã]o", "due to a signalling fault"),
    (r"devido\s+a\s+avaria\s+(?:num|de)\s+comboio", "due to a train fault"),
    (r"devido\s+a\s+avaria", "due to a fault"),
    (r"n[aã]o\s+[eé]\s+poss[ií]vel\s+prever\s+a\s+dura[cç][aã]o\s+da\s+interrup[cç][aã]o", "its duration cannot be predicted"),
    (r"tempos\s+de\s+espera\s+superiores\s+ao\s+normal", "longer waiting times than usual"),
)
...
def _interrupted_route_segments(
    ...
    """Return the Metro lines whose ridden stretch on this route is stopped now.

    Args:
        first_line_id: Line boarded at the first station.
        second_line_id: Line taken after the transfer, if any.
        board_station: Boarding station.
        transfer_station: Transfer station, if any.
        final_station: Exit station.

    Returns:
        Line identifiers (for example ``["amarela"]``) whose live status stops
        trains on the section the route rides; empty when the route is clear.
    """
    from tools.transport_api import metro_segment_interruption

    blocked: List[str] = []
    if first_line_id and board_station and metro_segment_interruption(
        first_line_id, board_station, transfer_station or final_station or ""
    ):
        blocked.append(first_line_id)
    if second_line_id and transfer_station and final_station and metro_segment_interruption(
        second_line_id, transfer_station, final_station
    ):
        blocked.append(second_line_id)
    return blocked
...
def _find_metro_path_avoiding_current_disruptions(origin: str, destination: str) -> Optional[List[Tuple[str, str, str]]]:
    ...
    for line_id, line_info in METRO_LINES.items():
        ...
        for station_a, station_b in zip(stations, stations[1:], strict=False):
            if metro_segment_interruption(line_id, station_a, station_b):
                continue
            graph.setdefault(station_a, []).append((station_b, line_id))
            graph.setdefault(station_b, []).append((station_a, line_id))

    # Cost in minutes, as the route estimate counts it: about 2 min per
    # station and 3 min per transfer. Fewer transfers break ties.
    ...
    while queue:
        minutes, transfers, station, current_line, path = heapq.heappop(queue)
        if station == end:
            return path
        for next_station, line_id in graph.get(station, []):
            changes = 1 if current_line and current_line != line_id else 0
            next_cost = (minutes + 2 + 3 * changes, transfers + changes)
            state_key = (next_station, line_id)
            if best_cost.get(state_key, (10_000, 10_000)) <= next_cost:
                continue
...
def _build_disruption_safe_metro_route(
    ...
    blocked_lines: Optional[List[str]] = None,
    usual_time: Optional[str] = None,
) -> Optional[str]:
    """Build a Metro alternative that avoids the stopped sections of the usual route.

    Args:
        origin: User-facing origin.
        destination: User-facing destination.
        language: Output language (``pt`` or ``en``).
        board_station: Boarding station of the usual route.
        final_station: Exit station of the usual route.
        blocked_lines: Lines of the usual route that are stopped now.
        usual_time: Estimated time of the usual route, to say what the
            detour costs.

    Returns:
        The alternative route answer, or ``None`` when no Metro path avoids
    ...
    blocked_lines = [line_id for line_id in (blocked_lines or []) if line_id]
    state_lines = _build_route_state_lines([*blocked_lines, *used_lines], language)
    blocked_names = " e ".join(_line_display_name(line_id, "pt") for line_id in blocked_lines) or "uma linha"
    blocked_names_en = " and ".join(_line_display_name(line_id, "en") for line_id in blocked_lines) or "a line"
    usual_pt = f" (seria cerca de {usual_time.strip('~ ')})" if usual_time else ""
    usual_en = f" (about {usual_time.strip('~ ')})" if usual_time else ""
    ...
    if language == "pt":
        lines = [
            ...
            "",
            f"✅ **Resposta direta:** a rota mais rápida usaria a **{blocked_names}**{usual_pt}, mas essa linha tem a circulação interrompida neste momento; por isso recomendo a alternativa de Metro abaixo, que a evita.",
            "",
            "---",
            "",
    ...
    lines = [
        f"### 🚇 **{_get_transport_display_name(origin)} → {_get_transport_display_name(destination)}**",
        "",
        f"✅ **Direct answer:** the fastest route would use the **{blocked_names_en}**{usual_en}, but service on it is interrupted right now, so I recommend the Metro alternative below, which avoids it.",
        "",
        "---",
        "",
        _metro_state_title(state_lines, language),
...
def _build_carris_direct_option(
    ...
    max_wait_minutes = 45
    max_single_walk_minutes = 8
    max_total_walk_minutes = 12
    ...
    route_matches = list(
        re.finditer(
            r"^\s*(?P<route>\d{1,4}[A-Z]?):\s+para\s+(?P<headsign>[^\n]+)",
            bus_section,
            flags=re.MULTILINE,
        )
    )
    ...
    candidates: List[Dict[str, Any]] = []
    for idx, route_match in enumerate(route_matches):
        block_start = route_match.start()
        block_end = route_matches[idx + 1].start() if idx + 1 < len(route_matches) else len(bus_section)
        block = bus_section[block_start:block_end]

        next_match = re.search(r"^\s*Next:\s*(?P<times>[^\n]+?)\s+\(stop (?P<stop>[^\)]+)\)", block, flags=re.MULTILINE)
        travel_match = re.search(r"^\s*~(?P<travel>\d+)min travel", block, flags=re.MULTILINE)
        stops_match = re.search(
            r"^\s*Stops:\s*board at (?P<board>.+?);\s*leave at stop (?P<leave>.+?)\.",
            block,
            flags=re.MULTILINE,
        )
        initial_walk_match = re.search(r"Caminhada inicial:\s*~(?P<walk>\d+)\s*min", block)
        final_walk_match = re.search(r"Caminhada final:\s*~(?P<walk>\d+)\s*min", block)
        if not next_match or not travel_match or not stops_match:
            continue
        ...
        wait_minutes = _minutes_until_clock_time(departure_clock)
        travel_minutes = int(travel_match.group("travel"))
        initial_walk = int(initial_walk_match.group("walk")) if initial_walk_match else 0
        final_walk = int(final_walk_match.group("walk")) if final_walk_match else 0
        total_walk = initial_walk + final_walk

        # "Other sensible options" must be usable now and close enough to the
        # requested endpoints. A bus that starts after a long wait or requires
        # long access walks should not be presented as a good alternative to a
        # validated primary route.
        if wait_minutes is None or wait_minutes > max_wait_minutes:
            continue
        if initial_walk > max_single_walk_minutes or final_walk > max_single_walk_minutes:
            continue
        if total_walk > max_total_walk_minutes:
            continue

        score = wait_minutes + travel_minutes + total_walk
        candidates.append(
            {
                "score": score,
                "route": route_match.group("route").strip(),
                "departure": departure_clock,
    ...
    route_code = best["route"]
    departure_clock = best["departure"]
    travel_minutes = best["travel"]
    initial_walk = best["initial_walk"]
    final_walk = best["final_walk"]
    board_stop = best["board"]
    leave_stop = best["leave"]
    destination_label = _get_transport_display_name(destination)

    if language == "pt":
        summary = (
            f"- 🚌 **Autocarro {route_code}**: próxima saída às {departure_clock}; "
            f"embarque em **{board_stop}** e saia em **{leave_stop}**"
        )
        walk_parts = []
        if initial_walk:
            walk_parts.append(f"~{initial_walk} min a pé até à paragem")
        if final_walk:
            walk_parts.append(f"~{final_walk} min a pé até {destination_label}")
        if walk_parts:
            summary += " (" + " + ".join(walk_parts) + ")"
        summary += f", ~{travel_minutes} min no autocarro."
    else:
        summary = (
            f"- 🚌 **Bus {route_code}**: next departure at {departure_clock}; "
            f"board at **{board_stop}** and alight at **{leave_stop}**"
        )
        walk_parts = []
        if initial_walk:
            walk_parts.append(f"about {initial_walk} min walk to the stop")
        if final_walk:
            walk_parts.append(f"about {final_walk} min walk to {destination_label}")
        if walk_parts:
            summary += " (" + " + ".join(walk_parts) + ")"
        summary += f", about {travel_minutes} min on the bus."

    return int(best["score"]), summary
    ...
    *,
    ...
    for raw_line in raw.splitlines():
        ...
        if group_match:
            ...
            continue
        times_match = re.search(r"(?:🕒|🕒)\s*(?P<times>.+)", stripped)
...
def _query_asks_cp_airport_access(user_message: str) -> bool:
    """Return whether the user asks if CP/train lines serve Lisbon Airport."""
    normalized = _normalize_token(user_message)
    has_train = bool(re.search(r"\b(?:cp|comboio|comboios|train|trains)\b", normalized))
    has_airport = bool(re.search(r"\b(?:aeroporto|airport)\b", normalized))
    asks_access = bool(
        re.search(
            r"\b(?:serve|serves|served|servem|serve|passam|passa|lines?|linhas?|"
            r"stations?|esta[cç][oõ]es?|closest|nearest|near|perto)\b",
            normalized,
        )
    )
    return has_train and has_airport and asks_access


def _build_cp_airport_access_response(language: str = "pt") -> str:
    """Build a grounded limitation for CP/train access to Lisbon Airport."""
    timestamp = datetime.now().strftime("%H:%M")
    if (language or "").lower().startswith("pt"):
        return "\n".join(
            [
                "### 🚆 **Comboio e Aeroporto de Lisboa**",
                "",
                "✅ **Resposta direta:** nenhuma linha suburbana da **CP** serve diretamente o Aeroporto de Lisboa.",
                "",
                "---",
                "",
                "- 🚇 Para acesso ferroviário direto ao terminal, usa o **Metro de Lisboa — Linha Vermelha**, estação **Aeroporto**.",
                "- 🚆 Se vieres de CP, a interface ferroviária pesada mais próxima e útil é **Oriente**; daí fazes transbordo para a Linha Vermelha até **Aeroporto**.",
                "- Não trato Oriente como uma estação CP dentro do aeroporto: é uma ligação com transbordo.",
                "",
                f"📌 **Fonte:** [*Metro de Lisboa*](https://www.metrolisboa.pt) | [*CP*](https://www.cp.pt) | **Atualizado:** {timestamp}",
            ]
        ).strip()
    return "\n".join(
        [
            "### 🚆 **Train access to Lisbon Airport**",
            "",
            "✅ **Direct answer:** no **CP** suburban rail line serves Lisbon Airport directly.",
            "",
            "---",
            "",
            "- 🚇 For direct rail access to the terminal, use **Metro de Lisboa — Red Line**, station **Aeroporto**.",
            "- 🚆 If you arrive by CP, the closest useful heavy-rail interchange is **Oriente**; from there, transfer to the Metro Red Line to **Aeroporto**.",
            "- I am not treating Oriente as an airport CP station: it is a transfer connection.",
            "",
            f"📌 **Source:** [*Metro de Lisboa*](https://www.metrolisboa.pt) | [*CP*](https://www.cp.pt) | **Updated:** {timestamp}",
        ]
    ).strip()
...
_CP_GENERIC_LISBON_TERMINAL_BY_LINE = {
...
def _is_generic_lisbon_cp_endpoint(fragment: str) -> bool:
    """Return whether a CP endpoint names Lisbon city rather than a station."""
    normalized = _normalize_token(_clean_query_fragment(fragment or ""))
    return normalized in _GENERIC_LISBON_CP_ENDPOINTS


def _resolve_cp_trip_endpoint(fragment: str, other_fragment: str = "") -> str:
    """Resolve a CP endpoint, anchoring generic Lisbon to the matching line hub.

    Requests such as "Cascais -> Lisboa de comboio" usually mean the Lisbon
    terminal of the relevant CP suburban line, not the fuzzy station that happens
    to score highest for the token "Lisboa". The anchor is selected from the
    other endpoint's confirmed CP line when available; otherwise the existing CP
    station resolver is preserved.
    """
    if not _is_generic_lisbon_cp_endpoint(fragment):
        return _resolve_cp_station_name(fragment)

    other_station = _resolve_cp_station_name(other_fragment)
    try:
        from tools.cp_api import get_cp_station_info
    except Exception:
        return _resolve_cp_station_name(fragment)

    other_info = get_cp_station_info(other_station)
    if not other_info:
        return _resolve_cp_station_name(fragment)

    for line_key in ("cascais", "sintra", "azambuja"):
        if line_key not in (other_info.get("lines") or []):
            continue
        anchor = _CP_GENERIC_LISBON_TERMINAL_BY_LINE[line_key]
        if _normalize_token(other_station) != _normalize_token(anchor):
            return anchor
...
def _build_cp_tool_spec(user_message: str) -> Optional[Dict[str, Any]]:
    ...
    if endpoints and explicit_train_context and len(endpoints) >= 2 and endpoints[1]:
        return {
            "name": "plan_train_trip",
            "args": {
                "origin": _resolve_cp_trip_endpoint(endpoints[0], endpoints[1]),
                "destination": _resolve_cp_trip_endpoint(endpoints[1], endpoints[0]),
            },
        }
...
def _flag_interrupted_metro_route(
    ...
    *,
    ...
    """Say, in the direct answer, that the Metro route rides a line stopped now.

    Used when no Metro path avoids the stopped section. The route stays below
    because it is still the fastest once service resumes and the user may
    have asked for the Metro only; the direct answer must not present it as
    usable now. For a trip on a later day, only a note is added, since the
    current stop may be over by then.

    Args:
        response_lines: Answer lines built so far (title and direct answer).
        blocked_lines: Stopped Metro line identifiers on the route.
        estimated_time: Estimated time of the route, if known.
        language: Output language (``pt`` or ``en``).
        future_planning: Whether the trip is for a later time.

    Returns:
        The updated answer lines.
    """
    is_pt = language == "pt"
    names = (" e " if is_pt else " and ").join(_line_display_name(line_id, language) for line_id in blocked_lines)
    if future_planning:
        note = (
            f"⚠️ **Atenção:** neste momento a **{names}** tem a circulação interrompida; confirma o estado da linha antes de saíres."
            if is_pt
            else f"⚠️ **Note:** service on the **{names}** is interrupted right now; check the line status before you leave."
        )
        insert_at = next((index for index, line in enumerate(response_lines) if line.strip() == "---"), len(response_lines))
        return [*response_lines[:insert_at], note, "", *response_lines[insert_at:]]
    duration = f" (cerca de {estimated_time.strip('~ ')})" if is_pt and estimated_time else (
        f" (about {estimated_time.strip('~ ')})" if estimated_time else ""
    )
    direct = (
        f"✅ **Resposta direta:** a rota de Metro seria pela **{names}**{duration}, mas essa linha tem a circulação "
        "interrompida neste momento e não há alternativa de Metro que a evite; confirma a retoma antes de saíres "
        "ou usa outro transporte."
        if is_pt
        else f"✅ **Direct answer:** the Metro route would use the **{names}**{duration}, but service on it is "
        "interrupted right now and no Metro alternative avoids it; check that service has resumed before you leave "
        "or use another mode."
    )
    replaced = False
    updated: List[str] = []
    for line in response_lines:
        if not replaced and re.match(r"^✅\s+\*\*(?:Resposta direta|Direct answer):\*\*", line.strip()):
            updated.append(direct)
            replaced = True
        else:
            updated.append(line)
    if not replaced:
        updated.extend([direct, "", "---", ""])
    return updated
...
def _build_deterministic_metro_route_response(
    ...
    # A route that rides a stopped section is replaced by a Metro path that
    # avoids it; when none exists the usual route stays, flagged as stopped,
    # so a caller with other modes can recommend them instead.
    blocked_lines = _interrupted_route_segments(
        first_line_id,
        second_line_id,
        board_station,
        transfer_station,
    ...
    if blocked_lines and not future_planning:
    ...
    if blocked_lines:
        response_lines = _flag_interrupted_metro_route(
            response_lines, blocked_lines, estimated_time, language, future_planning=future_planning
        )
...
def _build_deterministic_route_tool_response(user_message: str, language: Optional[str] = None) -> Optional[str]:
    ...
    # "Metro or bus?" compares modes: the bus-only shortcut would drop the Metro.
    modes_compared = bool(
        _parse_route_mode_preferences(user_message).get("alternative_mode_request")
        and re.search(r"\bmetro\b", user_message, flags=re.IGNORECASE)
    )

    if bus_or_tram_requested and not modes_compared:
        try:
            from tools.carris_api import carris_find_routes_between

            carris_result = str(
                carris_find_routes_between.invoke(
                    _carris_route_tool_args(endpoints[0], endpoints[1], user_message)
                )
            ).strip()
        except Exception:
            carris_result = ""
    ...
    if valid_carris_result and _is_generic_public_transport_route_query(user_message) and is_metro_route:
        ...
        # Place names as typed ("aeroporto") start with a capital in a title.
        title_origin, title_destination = (name[:1].upper() + name[1:] for name in endpoints[:2])
        title = f"### 🚇🚌 **{title_origin} → {title_destination}**"
        ...
        concrete_note = _metro_bus_direct_answer(
            metro_response, formatted_carris_response, _infer_language(user_message, "")
        )
        if concrete_note and not fastest_requested:
            first_note = concrete_note
        return (
            f"{title}\n\n"
            f"{first_note}\n\n"
            "---\n\n"
...
def _metro_bus_direct_answer(metro_response: str, carris_response: str, language: str) -> str:
    """Compare the Metro route and the best Carris option in one direct-answer sentence.

    Returns "" when the Metro duration or the bus line cannot be read, so the
    caller keeps its generic sentence. A bus option without a travel time is
    named without a comparison.
    """
    metro_minutes = re.search(
        r"(?:Tempo total estimado|Estimated total time|Tempo estimado|Estimated time)\D{0,20}?~?\s*(\d+)\s*min",
        metro_response or "",
    )
    transfer = re.search(r"(?:Transferência em|Transfer at)\s+([^(\n*]+?)\s*\(", metro_response or "")
    bus = re.search(
        r"(?:Melhor opção confirmada|Best confirmed option):\*\*\s*(?:apanha o|take)\s+\*\*(?P<line>[^*]+)\*\*"
        r"(?:[^·\n]*·\s*\*\*~?\s*(?P<minutes>\d+)\s*min)?",
        carris_response or "",
    )
    if not (metro_minutes and bus):
        return ""
    metro_value = int(metro_minutes.group(1))
    bus_value = int(bus.group("minutes")) if bus.group("minutes") else None
    line = bus.group("line").strip()
    if bus_value is not None:
        # The bus time is in-vehicle only; add its walks to and from the stops
        # so it compares with the Metro's total.
        option = re.search(
            rf"(?ms)^-\s+\*\*{re.escape(line)}\*\*:.*?(?=^-\s+\*\*|\Z)",
            carris_response or "",
        )
        walks = re.findall(
            r"\*\*(?:Caminhada inicial|Caminhada final|Initial walk|Final walk):\*\*\s*~?\s*(\d+)\s*min",
            option.group(0) if option else "",
            flags=re.IGNORECASE,
        )
        bus_value += sum(int(minutes) for minutes in walks)
    # The Metro answer says when its route rides a stopped line: either it
    # already avoids it (a detour) or no Metro path does (the bus wins).
    stopped = re.search(
        r"(?:seria pela|usaria a|would use the)\s+\*\*(?P<names>[^*]+)\*\*", metro_response or ""
    )
    metro_detour = bool(stopped and re.search(r"que a evita|which avoids it", metro_response or ""))
    if stopped and not metro_detour:
        names = stopped.group("names").strip()
        bus_time = f" (cerca de {bus_value} min)" if bus_value is not None and language == "pt" else (
            f" (about {bus_value} min)" if bus_value is not None else ""
        )
        metro_faster = bus_value is None or metro_value < bus_value
        if language == "pt":
            metro_part = (
                f"o Metro seria mais rápido (cerca de {metro_value} min), mas a **{names}** tem a circulação interrompida neste momento"
                if metro_faster
                else f"o Metro (cerca de {metro_value} min) usaria a **{names}**, que tem a circulação interrompida neste momento"
            )
            return f"✅ **Resposta direta:** recomendo o autocarro **{line}**{bus_time}: {metro_part}."
        metro_part = (
            f"the Metro would be faster (about {metro_value} min), but service on the **{names}** is interrupted right now"
            if metro_faster
            else f"the Metro (about {metro_value} min) would use the **{names}**, where service is interrupted right now"
        )
        return f"✅ **Direct answer:** take bus **{line}**{bus_time}: {metro_part}."
    detour_pt = (
        f" A rota de Metro indicada evita a **{stopped.group('names').strip()}**, interrompida neste momento."
        if metro_detour
        else ""
    )
    detour_en = (
        f" The Metro route shown avoids the **{stopped.group('names').strip()}**, which is interrupted right now."
        if metro_detour
        else ""
    )
    if language == "pt":
        via = f" (com transbordo em {transfer.group(1).strip()})" if transfer else ""
        if bus_value is None:
            return (
                f"✅ **Resposta direta:** De Metro demora cerca de {metro_value} min{via}; "
                f"também há a opção de autocarro **{line}**, sem tempo de viagem confirmado."
            )
        faster = "o Metro é a opção mais rápida" if metro_value <= bus_value else f"o autocarro {line} é a opção mais rápida"
        return (
            f"✅ **Resposta direta:** De Metro demora cerca de {metro_value} min{via}; de autocarro, o **{line}** "
            f"demora cerca de {bus_value} min, por isso {faster}.{detour_pt}"
        )
    via = f" (change at {transfer.group(1).strip()})" if transfer else ""
    if bus_value is None:
        return (
            f"✅ **Direct answer:** By Metro it takes about {metro_value} min{via}; "
            f"there is also a bus option with the **{line}**, with no confirmed travel time."
        )
    faster = "the Metro is faster" if metro_value <= bus_value else f"bus {line} is faster"
    return (
        f"✅ **Direct answer:** By Metro it takes about {metro_value} min{via}; by bus, the **{line}** "
        f"takes about {bus_value} min, so {faster}.{detour_en}"
    )


def _build_deterministic_sequence_route_response(
    user_message: str,
    language: str,
) -> Optional[str]:
    """Build adjacent route-leg evidence for explicit multi-stop route requests."""
    try:
        from agent.agents.planner_agent import (
            ...
            _query_requests_movement_details,
            _query_requests_public_transport,
    ...
    if not (
        ...
        or re.search(r"\b(?:transportes?|desloca|como\s+vou|how\s+to\s+move|get\s+around)\b", user_message, re.IGNORECASE)
    ):
        return None
    ...
    is_pt = language == "pt"
    bullets: List[str] = []
    for origin, destination in zip(labels, labels[1:5]):
        route_prompt = (
            f"Como vou de {origin} para {destination} em transportes públicos?"
            if is_pt
            else f"How do I get from {origin} to {destination} by public transport?"
        )
        leg_output = _build_deterministic_route_tool_response(route_prompt)
        if not leg_output:
            continue
    ...
    direct = (
        "✅ **Resposta direta:** confirmei as ligações adjacentes entre os pontos pedidos."
        if is_pt
        else "✅ **Direct answer:** I confirmed the adjacent legs between the requested points."
    )
    body = "\n".join(bullets)
    source_entries: List[str] = []
    normalized_body = _normalize_token(body)
    if re.search(r"\b(?:metro|linha\s+(?:vermelha|verde|azul|amarela))\b", normalized_body):
        source_entries.append("[*Metro de Lisboa*](https://www.metrolisboa.pt)")
    if re.search(r"\b(?:carris|autocarro|bus|tram|eletrico|el[eé]trico)\b", normalized_body):
        source_entries.append("[*Carris*](https://www.carris.pt)")
    ...
    return (
        ...
        "---\n\n"
        f"{body}\n\n"
        f"📌 **{source_label}:** {' | '.join(dict.fromkeys(source_entries))} | **{updated_label}:** {timestamp}"
    )
...
def _build_deterministic_transport_tool_call(user_message: str) -> Optional[AIMessage]:
    ...
    for spec_builder in (
        ...
        if spec:
            if spec.get("name") in ("get_carris_metropolitana_alerts", "carris_get_service_frequency"):
...
class TransportAgent(BaseAgent):
    ...
    def resolve_itinerary_legs(
        ...
        *,
        ...
        """Resolve the movement between consecutive itinerary stops.

        The Planner chooses the stops; this worker then checks how to move
        between each consecutive pair with the transport tools. Stops within
        walking distance get a walking leg with the measured distance. Longer
        legs get a Metro route between the nearest stations, or a direct
        Carris tram/bus when the Metro does not serve both ends. A leg the
        tools cannot confirm comes back as an empty string, so the Planner
        states the limitation instead of inventing a line.

        Args:
            legs: Consecutive stop pairs chosen by the Planner.
            language: Output language (``pt`` or ``en``).
            mode: Requested transport mode from the planning brief.

        Returns:
            One Markdown movement line per leg, in the same order.
        """
        from concurrent.futures import ThreadPoolExecutor

        from agent.planning.legs import (
            MAX_FALLBACK_WALK_KM,
            WALKABLE_DISTANCE_KM,
            address_variants,
            extract_carris_options,
            format_carris_leg,
            format_walking_leg,
            metro_route_interrupted_lines,
            summarize_metro_route,
        )
        from tools.location_resolver import resolve_coordinates, resolve_location_query
        from tools.utils import haversine_distance

        metro_tool = self._get_tool_by_name("get_route_between_stations")
        carris_tool = self._get_tool_by_name("carris_find_routes_between")
        max_station_walk_km = 1.2

        def name_words(text: str) -> set:
        ...
        def same_name(label: str, found: str) -> bool:
            # "Cervejaria Portugália" names "Portugália Cervejaria Belém";
            # "DejaVu Plaza" does not name "Café Plaza".
            wanted, got = name_words(label), name_words(found)
            matched = len(wanted & got)
            return matched == len(wanted) or (matched >= 2 and len(wanted) - matched <= 1)

        def locate(query: str, label: str) -> Optional[Dict[str, Any]]:
            # A stop is located by its full address, then by its name, then by
            # simpler forms of the address (a street without a number is only
            # approximate). A name result is kept only when it names the stop:
            # a name search can return another place ("DejaVu Plaza" for "Café Plaza").
            has_address = bool(query and query != label)
            variants = address_variants(query) if has_address else []
            queries = [*variants[:1], label, *variants[1:]] if has_address else [label]
            for candidate in queries:
                if not candidate:
                    continue
                try:
                    resolved = resolve_location_query(candidate)
                except Exception as exc:
                    logger.debug("Leg endpoint lookup failed for %r: %s", candidate, exc)
                    continue
                if not (resolved.get("success") and resolved.get("lat") is not None and resolved.get("lon") is not None):
                    continue
                if has_address and candidate == label and not same_name(label, str(resolved.get("display_name") or "")):
                    continue
                return resolved
            return None

        def station_walk_km(station: Dict[str, Any]) -> float:
            distance = station.get("distance_km")
            return float(distance) if distance is not None else 99.0

        def resolve(leg: "LegRequest") -> str:
            start = (
                resolve_coordinates(*leg.origin_coords, label=leg.origin_label)
                if leg.origin_coords
                else locate(leg.origin_query, leg.origin_label)
            )
            end = (
                resolve_coordinates(*leg.destination_coords, label=leg.destination_label)
                if leg.destination_coords
                else locate(leg.destination_query, leg.destination_label)
            )
            if not start or not end:
                return ""
            distance_km = haversine_distance(start["lat"], start["lon"], end["lat"], end["lon"])
            walking_leg = format_walking_leg(leg.origin_label, leg.destination_label, distance_km, language)
            # A short hop is walked; the start leg tries the requested mode first
            # ("which tram should I take from Martim Moniz?"), unless the stop is
            # only a few minutes away.
            try_transit_first = leg.prefer_transit and mode in {"metro", "tram", "bus"} and distance_km > 0.3
            if leg.walk_only or mode == "walking" or (distance_km <= WALKABLE_DISTANCE_KM and not try_transit_first):
                return walking_leg

            interrupted_metro_leg = ""
            if metro_tool is not None and mode in {"", "metro", "public_transport"}:
                start_station = start.get("nearest_metro") or {}
                end_station = end.get("nearest_metro") or {}
                if (
                    start_station.get("name")
                    and end_station.get("name")
                    and start_station["name"] != end_station["name"]
                    and station_walk_km(start_station) <= max_station_walk_km
                    and station_walk_km(end_station) <= max_station_walk_km
                ):
                    route_text = str(
                        self._invoke_tool(
                            metro_tool,
                            {"origin": start_station["name"], "destination": end_station["name"]},
                            tool_name="get_route_between_stations",
                        )
                        or ""
                    )
                    metro_leg = summarize_metro_route(
                        route_text,
                        leg.origin_label,
                        leg.destination_label,
                        language,
                        walk_before_km=station_walk_km(start_station),
                        walk_after_km=station_walk_km(end_station),
                    )
                    # A Metro leg on a stopped line is kept only when the user
                    # asked for the Metro or no bus/tram connects the stops.
                    if metro_leg and (mode == "metro" or not metro_route_interrupted_lines(route_text)):
                        return metro_leg
                    interrupted_metro_leg = metro_leg

            if carris_tool is not None and mode in {"", "tram", "bus", "public_transport"}:
                # Resolved endpoints go to the tool as coordinates, so the
                # route is searched from the same points the leg was measured on.
                route_text = str(
                    self._invoke_tool(
                        carris_tool,
                        {
                            "origin": f"{start['lat']:.6f},{start['lon']:.6f}",
                            "destination": f"{end['lat']:.6f},{end['lon']:.6f}",
                ...
                options = extract_carris_options(route_text)
                # The requested vehicle first; the other one only as a stated fallback.
                if mode == "tram":
                    options = [option for option in options if option.is_tram] or options
                elif mode == "bus":
                    options = [option for option in options if not option.is_tram] or options
                carris_leg = format_carris_leg(
                    options, leg.origin_label, leg.destination_label, language, requested_mode=mode
                )
                if carris_leg:
                    return carris_leg
            if distance_km <= MAX_FALLBACK_WALK_KM:
                return walking_leg
            return interrupted_metro_leg

        if not legs:
            return []
        with ThreadPoolExecutor(max_workers=min(4, len(legs))) as executor:
            return list(executor.map(resolve, legs))

    def _metropolitana_bus_result_with_fallback(
        self, origin: str, destination: str, prefer_metropolitana: bool
    ) -> str:
        """Resolves a Carris Metropolitana direct-bus lookup with a tool fallback.

        The two metropolitana route tools are complementary: ``find_bus_routes``
        resolves precise stop-to-stop geometry but has low recall for bare
        municipality names (e.g. "Loures"), while ``find_direct_bus_lines``
        matches lines by served localities and has high recall. Committing to
        only one tool (per ``prefer_metropolitana``) produced false
        "no direct bus" answers for broad-municipality pairs that the other tool
        answers correctly. This tries the preferred tool first and falls back to
        the other when the first reports no direct match, mirroring the worker
        prompt's "call both before concluding there is no bus option" rule.

        Args:
            origin: Origin location name.
            destination: Destination location name.
            prefer_metropolitana: Whether the suburban/precise tool is preferred.

        Returns:
            str: The first matched tool result, or the first non-empty result
            (preserving the downstream "no match" message) when neither matches.
        """
        preferred = "find_bus_routes" if prefer_metropolitana else "find_direct_bus_lines"
        fallback = "find_direct_bus_lines" if prefer_metropolitana else "find_bus_routes"
        first_result = ""
        for tool_name in (preferred, fallback):
            ...
            try:
                ...
                continue
            if result and not _tool_result_indicates_no_match(result):
                return result
            first_result = first_result or result
        return first_result

    def _build_cp_leg_without_metro_access_limitation(
        self,
        origin: str,
        destination: str,
        user_message: str,
        ...
        """Return an honest CP leg when a full non-metro access route is unavailable."""
        try:
            from tools.cp_api import get_cp_station_info, plan_train_trip
        except Exception:
            return None
        ...
        if origin_cp:
            candidate_origins = [str(origin_cp.get("name") or origin).strip()]
        else:
            candidate_origins = [
                "Entrecampos",
                "Sete Rios",
        ...
        selected_station = ""
        selected_result = ""
        for station in candidate_origins:
            if not station or _normalize_token(station) == _normalize_token(destination_station):
                continue
            try:
                candidate = str(
                    self._invoke_tool(
                        plan_train_trip,
                        {"origin": station, "destination": destination_station},
                        tool_name="plan_train_trip",
                    )
                ).strip()
            except Exception:
                continue
            if candidate and not _tool_result_indicates_no_match(candidate):
                selected_station = station
                selected_result = candidate
                break

        if not selected_station or not selected_result:
            return None

        formatted_train = self._format_deterministic_tool_result(
            tool_name="plan_train_trip",
            tool_args={"origin": selected_station, "destination": destination_station},
            result=selected_result,
            language=language,
            user_message=user_message,
        )
        train_body = _strip_transport_source_lines(formatted_train).strip()
        train_body = re.sub(
            r"(?mis)^###\s+🚆\s+\*\*[^*\n]+\*\*\s*\n+"
            r"(?:✅|⚠️)\s+\*\*(?:Resposta direta|Direct answer):\*\*\s*",
            "",
            train_body,
        ).strip()
        origin_display = _get_transport_display_name(origin)
        destination_display = _get_transport_display_name(destination)
        station_display = _get_transport_display_name(selected_station)
        if language == "pt":
            return "\n".join(
                [
                    f"### 🚆 **{origin_display} → {destination_display} sem metro**",
                    "",
                    "⚠️ **Resposta direta:** não consegui confirmar uma rota completa **sem Metro** porta-a-porta com os dados de superfície disponíveis; "
                    f"a perna ferroviária confirmada é **{station_display} → {destination_display}** pela CP.",
                    "",
                    "---",
                    "",
                    "### 🚆 **Perna CP confirmada**",
                    "",
                    train_body or selected_result,
                    "",
                    "### 🚶🚌 **Acesso sem Metro por confirmar**",
                    "",
                    f"- Para cumprir o pedido **sem Metro**, ainda tens de chegar de **{origin_display}** a **{station_display}** a pé ou por autocarro.",
                    "- Não invento essa ligação de superfície quando as tools não a confirmaram.",
                    "",
        ...
        return "\n".join(
            [
                f"### 🚆 **{origin_display} → {destination_display} without Metro**",
                "",
                "⚠️ **Direct answer:** I could not confirm a full door-to-door route **without Metro** using the available surface data; "
                f"the confirmed rail leg is **{station_display} → {destination_display}** on CP.",
                "",
                "---",
                "",
                "### 🚆 **Confirmed CP leg**",
                ...
                train_body or selected_result,
                "",
                "### 🚶🚌 **Non-Metro access still unconfirmed**",
                "",
                f"- To satisfy **without Metro**, you still need to reach **{station_display}** from **{origin_display}** on foot or by bus.",
                "- I will not invent that surface connection when the tools did not confirm it.",
                "",
                self._build_transport_source_line(
                    language,
                    [
    ...
    def _build_less_walking_carris_route_response(
        self,
        origin: str,
        destination: str,
        user_message: str,
        ...
        """Build a deterministic Carris route ranked by lowest walking time."""
        ...
        try:
            result = str(
                self._invoke_tool(
                    urban_tool,
                    _carris_route_tool_args(origin, destination, user_message, search_radius_km=0.8),
                    tool_name="carris_find_routes_between",
                )
            ).strip()
        except Exception:
            return None
        if not result or _tool_result_indicates_no_match(result):
            return None

        candidates: List[Dict[str, Any]] = []
        for section_name, mode_label in (("BUSES", "autocarro"), ("TRAMS", "elétrico")):
            for entry in _parse_carris_route_entries(_extract_carris_mode_section(result, section_name)):
                details = [str(detail or "") for detail in entry.get("details", [])]
                walk_minutes = [
                    int(match.group("minutes"))
                    for detail in details
                    for match in re.finditer(
                        r"Caminhada (?:inicial|final):\s*~(?P<minutes>\d+)\s*min",
                        detail,
                        flags=re.IGNORECASE,
                    )
                ]
                ...
                travel_match = next(
                    (
                        re.search(r"~\s*(?P<travel>\d+)\s*min(?:\s*travel)?", detail, flags=re.IGNORECASE)
                        for detail in details
                        if re.search(r"~\s*\d+\s*min", detail, flags=re.IGNORECASE)
                    ),
                    None,
                )
                departure_match = next(
                    (
                        re.search(r"^(?:\*\*)?Next(?:\*\*)?:\s*(?P<body>.+)$", detail, flags=re.IGNORECASE)
                        for detail in details
                    ),
                    None,
                )
                candidates.append(
                    {
                        "entry": entry,
                        "mode": mode_label,
                        "total_walk": sum(walk_minutes),
                        "travel": int(travel_match.group("travel")) if travel_match else 999,
                        "has_departure": bool(departure_match),
                    }
                )

        if not candidates:
            return None
        candidates = [candidate for candidate in candidates if candidate["has_departure"]] or candidates
        best = min(
            candidates,
            key=lambda item: (
                int(item["total_walk"]),
                int(item["travel"]),
                str(item["entry"].get("route") or ""),
            ),
        )
        entry = best["entry"]
        route = str(entry.get("route") or "").strip()
        summary = str(entry.get("summary") or "").strip()
        mode_label = str(best["mode"])
        total_walk = int(best["total_walk"])
        detail_lines: List[str] = []
        for detail in entry.get("details", []):
            detail_lines.extend(_format_carris_route_detail(str(detail), language, route=route))
        timestamp = datetime.now().strftime("%H:%M")
        origin_display = _get_transport_display_name(origin)
        destination_display = _get_transport_display_name(destination)
        if language == "pt":
            title = f"### 🚌🚋 **{origin_display} → {destination_display}**"
            direct = (
                f"✅ **Resposta direta:** a opção confirmada com menos caminhada é a linha **{route}** "
                f"({mode_label}), com cerca de **~{total_walk} min de caminhada total**."
            )
            body = [
                f"- **Linha:** {route} — {summary}",
                f"- 🚶 **Caminhada total estimada:** ~{total_walk} min",
                *detail_lines,
            ]
            source = f"📌 **Fonte:** [*Carris*](https://www.carris.pt) | **Atualizado:** {timestamp}"
        else:
            title = f"### 🚌🚋 **{origin_display} → {destination_display}**"
            mode_en = "tram" if mode_label == "elétrico" else "bus"
            direct = (
                f"✅ **Direct answer:** the confirmed lowest-walking option is line **{route}** "
                f"({mode_en}), with about **~{total_walk} min total walking**."
            )
            body = [
                f"- **Line:** {route} — {summary}",
                f"- 🚶 **Estimated total walking:** ~{total_walk} min",
                *detail_lines,
            ]
            source = f"📌 **Source:** [*Carris*](https://www.carris.pt) | **Updated:** {timestamp}"
        return "\n".join([title, "", direct, "", "---", "", *body, "", source]).strip()
    ...
    def _preprocess_tool_args(self, tool_name: str, tool_args: dict) -> dict:
        ...
        if tool_name == "carris_find_routes_between" and "start_time" in cleaned:
            active_message = str(getattr(self, "_active_user_message", "") or "")
            if not _extract_requested_departure_clock(active_message):
    ...
    def _build_madeira_metro_destination_ambiguity_response(
        self,
        user_message: str,
        language: str,
    ) -> Optional[str]:
        """Clarify metro-to-Madeira requests before any route tool is called.

        "Madeira" can mean the autonomous-region island, which is outside the
        Lisbon Metro network, or a Lisbon address/street containing "Madeira".
        A metro route should not be computed until the user confirms which one.
        """
        normalized = _normalize_token(user_message)
        if "metro" not in normalized or "madeira" not in normalized:
            return None
        if not re.search(
            r"\b(?:ir|vou|chego|chegar|leva|rota|trajeto|percurso|apanhar|apanho|"
            r"go|get|route|journey|take|to)\b",
            normalized,
        ):
            return None
        if re.search(r"\b(?:rua|avenida|av|humberto|morada|address)\b", normalized):
            return None

        if language == "pt":
            return (
                "### ⚠️ **Destino ambíguo: Madeira**\n\n"
                "✅ **Resposta direta:** não vou calcular uma rota de metro até “Madeira” sem confirmar o destino.\n\n"
                "- Se queres dizer **Ilha da Madeira**, não existe ligação pelo Metro de Lisboa; é uma deslocação fora da rede urbana.\n"
                "- Se queres dizer uma **morada em Lisboa** com “Madeira” no nome, envia o nome completo da rua ou ponto de partida/destino.\n"
            )
        return (
            "### ⚠️ **Ambiguous destination: Madeira**\n\n"
            "✅ **Direct answer:** I will not calculate a metro route to “Madeira” until the destination is confirmed.\n\n"
            "- If you mean **Madeira island**, Lisbon Metro cannot take you there; it is outside the urban network.\n"
            "- If you mean a **Lisbon address** with “Madeira” in the name, send the full street name or exact origin/destination.\n"
        )
    ...
    def _is_mode_comparison_query(user_message: str) -> bool:
        ...
        asks_comparison = bool(
            ...
            # "de metro ou comboio?" offers the two modes as a choice.
            or re.search(r"\b(?:metro|comboios?|trains?|autocarros?|bus(?:es)?)\s+(?:ou|or|vs|versus)\s+(?:(?:de|o|a|the|by)\s+)?(?:metro|comboios?|trains?|autocarros?|bus(?:es)?)\b", normalized_query)
        )
    ...
    def _build_mode_comparison_response(
        ...
        arrow = "\u2192"
        # The comparison is the direct answer: which mode is faster, by how much.
        if metro_minutes is not None and train_minutes is not None:
            train_faster = train_minutes < metro_minutes
            if language == "pt":
                direct = (
                    f"o comboio é mais rápido (~{train_minutes} min) do que o Metro (~{metro_minutes} min)"
                    if train_faster
                    else f"o Metro é mais rápido (~{metro_minutes} min) do que o comboio (~{train_minutes} min)"
                )
            else:
                direct = (
                    f"the train is faster (~{train_minutes} min) than the Metro (~{metro_minutes} min)"
                    if train_faster
                    else f"the Metro is faster (~{metro_minutes} min) than the train (~{train_minutes} min)"
                )
        elif metro_minutes is not None:
            # Only the Metro is confirmed: say so, with its time.
            direct = (
                f"de Metro demora cerca de {metro_minutes} min; não confirmei uma ligação de comboio "
                f"entre {origin} e {destination}"
                if language == "pt"
                else f"by Metro it takes about {metro_minutes} min; I could not confirm a train connection "
                f"between {origin} and {destination}"
            )
        elif train_minutes is not None:
            direct = (
                f"de comboio demora cerca de {train_minutes} min; não confirmei uma rota de Metro "
                f"entre {origin} e {destination}"
                if language == "pt"
                else f"by train it takes about {train_minutes} min; I could not confirm a Metro route "
                f"between {origin} and {destination}"
            )
        else:
            direct = (
                "não foi possível comparar com segurança porque falta pelo menos uma duração oficial"
                if language == "pt"
                else "I could not compare confidently because at least one official duration is missing"
            )
        # A route on a stopped line is not "without disruption": the Metro
        # answer states it (interruption marker or flagged direct answer).
        metro_route_stopped = bool(
            re.search(
                r"interrompida neste momento|interrupted right now|circula[cç][aã]o interrompida|service interrupted|⛔",
                metro_response or "",
                flags=re.IGNORECASE,
            )
        )
        if asks_cheapest:
            direct += (
                "; o preço não foi confirmado nas fontes oficiais disponíveis"
                if language == "pt"
                else "; the fares could not be confirmed with the official sources available"
            )
        if language == "pt":
            lines = [
                f"### 🚇🚆 **Metro ou comboio: {origin} {arrow} {destination}**",
                "",
                f"✅ **Resposta direta:** {direct[0].upper() + direct[1:]}.",
                "",
                "---",
                "",
            ...
            if metro_route_stopped:
                lines.append("- ⚠️ **Estado:** a rota de Metro usa uma linha com circulação interrompida neste momento; confirma a retoma antes de saíres")
            elif "circulação normal" in metro_response.lower():
                lines.append("- ✅ **Estado:** linhas de Metro usadas sem perturbação reportada no momento da consulta")
            if metro_route_bullets:
                lines.extend(["- 🧭 **Trajeto Metro:**", *(f"    {bullet}" for bullet in metro_route_bullets)])
                if "sete rios" in _normalize_token(destination) and any(
                    ...
                    lines.append("- ℹ️ **Sete Rios no Metro:** a estação que serve Sete Rios chama-se **Jardim Zoológico**.")
            else:
            ...
            lines = [
                f"### 🚇🚆 **Metro or train: {origin} {arrow} {destination}**",
                "",
                f"✅ **Direct answer:** {direct[0].upper() + direct[1:]}.",
                "",
                "---",
                "",
            ...
            if metro_route_stopped:
                lines.append("- ⚠️ **Status:** the Metro route uses a line where service is interrupted right now; check that it has resumed before you leave")
            elif "normal service" in metro_response.lower() or "circulação normal" in metro_response.lower():
                lines.append("- ✅ **Status:** Metro lines used have no reported disruption at query time")
            if metro_route_bullets:
                lines.extend(["- 🧭 **Metro route:**", *(f"    {bullet}" for bullet in metro_route_bullets)])
                if "sete rios" in _normalize_token(destination) and any(
                    "Jardim Zoológico" in bullet or "Jardim Zoologico" in bullet
                    for bullet in metro_route_bullets
                ):
                    lines.append("- ℹ️ **Sete Rios by Metro:** the Metro station serving Sete Rios is **Jardim Zoológico**.")
            else:
    ...
    def _format_deterministic_tool_result(
        ...
        if tool_name == "carris_get_routes":
            if language == "pt":
                localized = cleaned_result
                localized = re.sub(r"Carris Urban route", "Rota Carris", localized, flags=re.IGNORECASE)
                localized = re.sub(r"(?m)^###\s+([^\n*]+)\*\*rota Carris\s+", r"### \1**Rota Carris ", localized)
                localized = localized.replace("- **Operator:** Carris Urban", "- **Operador:** Carris Urban")
                localized = localized.replace("- 🚌 **Operator:** Carris Urban", "- 🚌 **Operador:** Carris")
                localized = localized.replace("- 🔀 **Route variants:**", "- 🔀 **Variantes da linha:**")
                localized = re.sub(r"; and (\d+) more\b", r"; e mais \1", localized)
                localized = localized.replace(
                    "- **Source data:** official Carris GTFS static data (routes, trips, stop_times, stops)",
                    "- **Dados de origem:** dados oficiais da Carris",
                )
                localized = localized.replace(
                    "- **Source data:** official Carris data",
                    "- **Dados de origem:** dados oficiais da Carris",
                )
                localized = localized.replace("- **Route variants in GTFS:**", "- **Variantes da rota no GTFS:**")
                localized = localized.replace("- **Route variants:**", "- **Variantes da rota:**")
                localized = localized.replace("- **Representative stop sequences:**", "- **Sequências representativas de paragens:**")
                localized = re.sub(r"\((\d+) stops;", r"(\1 paragens;", localized)
                localized = re.sub(r"\.\.\. and (\d+) more variants", r"... e mais \1 variantes", localized)
                localized = localized.replace(
                    "- **Limitation:** these are representative GTFS trip stop sequences; live vehicle position and next departures require the real-time/departure tools.",
                    "- **Limitação:** estas são sequências representativas de paragens; a posição dos veículos em tempo real e as próximas partidas requerem consulta em tempo real.",
                )
                localized = localized.replace(
                    "- **Limitation:** these are representative stop sequences; live vehicle position and next departures require the real-time/departure tools.",
                    "- **Limitação:** estas são sequências representativas de paragens; a posição dos veículos em tempo real e as próximas partidas requerem consulta em tempo real.",
                )
                if "✅ **Resposta direta:**" not in localized and localized.startswith("### "):
                    route_match = re.search(r"Rota Carris\s+([A-Za-z0-9]+)", localized)
                    route_label = route_match.group(1) if route_match else "Carris"
                    variants = list(dict.fromkeys(
                        re.findall(r"(?m)^\s*-\s+(?:🚏\s+)?\*\*([^*\n]{3,80})\*\*\s+\(\d+\s+paragens;", localized)
                    ))
                    runs = f", que faz {' e '.join(variants[:2])}" if variants else ""
                    direct = (
                        f"✅ **Resposta direta:** a linha **{route_label}** da Carris{runs}; "
                        "as variantes e as paragens de cada sentido estão abaixo."
                    )
                    localized = re.sub(r"^(###\s+[^\n]+)\n+", rf"\1\n\n{direct}\n\n---\n\n", localized, count=1)
                return localized
            if "✅ **Direct answer:**" not in cleaned_result and cleaned_result.startswith("### "):
                # Name the line and where its variants run, not "a supported option".
                route_match = re.search(r"Carris Urban route\s+([A-Za-z0-9]+)", cleaned_result)
                route_label = route_match.group(1) if route_match else "Carris"
                variants = list(dict.fromkeys(re.findall(r"(?m)^\s*-\s+(?:🚏\s+)?\*\*([^*\n]{3,80})\*\*\s+\(\d+\s+stops;", cleaned_result)))
                runs = f", which runs {' and '.join(variants[:2])}" if variants else ""
                direct = (
                    f"✅ **Direct answer:** Carris line **{route_label}**{runs}; "
                    "its variants and representative stop sequences are listed below."
                )
        ...
        if tool_name == "plan_train_trip":
            ...
            next_arrival = departures[0][1] if departures else ""
            next_duration = departures[0][2] if departures else ""
            if status_value and language != "pt":
                # The CP tool states the live status in Portuguese.
                status_value = re.sub(
                    r"alguns comboios com \+(\d+) min de atraso", r"some trains running up to \1 min late", status_value
                )
                status_value = status_value.replace("comboios a horas no feed em tempo real", "trains on time in the live feed")
                status_value = status_value.replace("sem dados em tempo real", "no real-time data")
            if language == "pt":
                if line_value and next_departure:
                    direct_answer = (
                        f"o próximo comboio da **{line_value}** sai às **{next_departure}** e chega às "
                        f"**{next_arrival}** ({next_duration})"
                    )
                    direct_answer += f"; estado em tempo real: {status_value}." if status_value else "."
                elif line_value:
            ...
            if line_value and next_departure:
                direct_answer = (
                    f"the next **{line_value}** train leaves at **{next_departure}** and arrives at "
                    f"**{next_arrival}** ({next_duration})"
                )
                direct_answer += f"; live status: {status_value}." if status_value else "."
            elif line_value:
            ...
            if status_value:
                status_emoji = "🚦" if re.search(r"delay|late|atras", _normalize_token(status_value)) else "✅" if "on time" in status_value.lower() else "ℹ️"
        ...
        if tool_name == "get_metro_status":
            ...
            for emoji, raw_line_name, raw_status in line_matches:
                ...
                if "no disruption reported" in status_lower:
                    ...
                    for token in [
                        "circulação com atrasos",
                        "circulacao com atrasos",
                        "atrasos",
                        "delays",
                        "waiting time may be longer",
                        "tempo de espera pode ser superior",
                    ]
                ):
                    localized_status = (
                        "circulação com atrasos; o tempo de espera pode ser superior ao normal"
                        if language == "pt"
                        else "service with delays; waiting time may be longer than usual"
                    )
        ...
        if tool_name == "carris_get_realtime_vehicles":
            ...
            if active_entries:
                ...
                direct_line = (
                    f"✅ **Resposta direta:** o {route_short_name} está a circular, com **{total_active}** elétricos "
                    "ativos no feed em tempo real; o feed não inclui informação de atrasos, por isso não confirma "
                    "se a linha está a horas."
                    if language == "pt"
                    else f"✅ **Direct answer:** the {route_short_name} is running, with **{total_active}** trams active "
                    "in the live feed; the feed has no delay information, so it cannot confirm whether the line "
                    "is on time."
                )
                output_lines = [title, "", direct_line, "", "---", "", summary_line, active_line, direction_header]
    ...
    def _build_explicit_multi_mode_route_response(
        ...
        normalized_request = _normalize_token(user_message)
        metropolitana_only = bool(
            re.search(r"\b(?:carris\s+metropolitana|metropolitana|suburbano|suburbana|suburban|aml)\b", normalized_request)
            and not re.search(r"\bcarris\b(?!\s+metropolitana)|\b(?:carris\s+urbana?|carris\s+urban)\b", normalized_request)
        )
        ...
        urban_result = (
            str(
                self._invoke_tool(
                    urban_tool,
                    _carris_route_tool_args(origin, destination, user_message),
                    tool_name="carris_find_routes_between",
                )
            ).strip()
            if urban_tool and not metropolitana_only
            else ""
        )
        ...
        def frequency_lookup(route_short_name: str) -> str:
            ...
            try:
                return str(
                    self._invoke_tool(
                        frequency_tool,
                        {"route_short_name": route_short_name, "language": language},
        ...
        if urban_bus_markdown:
            ...
            recommended_option = _summarize_recommended_carris_option(
                urban_bus_markdown,
                language,
                scheduled_from=_extract_requested_departure_clock(user_message),
            )
            bus_body_parts = [part for part in [recommended_option, urban_bus_markdown] if part]
            bus_sections.append("\n\n".join(bus_body_parts))

        if prefer_metropolitana or not urban_bus_markdown:
            ...
            metropolitan_result = (
                self._metropolitana_bus_result_with_fallback(
                    origin, destination, prefer_metropolitana
                )
            ...
            elif prefer_metropolitana:
                source_links.append("[*Carris Metropolitana*](https://www.carrismetropolitana.pt)")
                bus_notes.append(
                    "- **Carris Metropolitana:** não foi confirmada uma linha suburbana direta para esta ligação."
                    if language == "pt"
                    else "- **Carris Metropolitana:** no direct suburban bus line was confirmed for this trip."
                )
        ...
        if bus_sections:
            bus_body = "\n\n".join(bus_sections)
            if bus_notes:
                notes_heading = "**ℹ️ Notas de cobertura**" if language == "pt" else "**ℹ️ Coverage notes**"
                bus_body = f"{bus_body}\n\n{notes_heading}\n" + "\n".join(bus_notes)
        ...
        if "train" in requested_modes:
            train_section_title = (
                "### 🚆 **Opção CP suburbano/AML**"
                if language == "pt"
                else "### 🚆 **CP suburban rail option**"
            )
            train_tool = self._get_tool_by_name("plan_train_trip")
            train_result = (
                str(
            ...
            source_links.append("[*CP*](https://www.cp.pt)")
            if train_result and not _tool_result_indicates_no_match(train_result):
                formatted_train = self._format_deterministic_tool_result(
                    tool_name="plan_train_trip",
                    tool_args={"origin": origin, "destination": destination},
                    result=train_result,
                ...
                sections.append(f"{train_section_title}\n\n{train_body or train_result}")
            else:
                limitation = (
                    f"- ⚠️ Não consegui confirmar uma ligação de **CP suburbano/AML** entre **{origin_display}** e **{destination_display}** nos dados disponíveis."
                    if language == "pt"
                    else f"- ⚠️ I couldn't confirm a **CP suburban rail** connection between **{origin_display}** and **{destination_display}** in the available data."
                )
                sections.append(f"{train_section_title}\n\n{limitation}")

        metro_confirmed = bool(metro_response)
        bus_confirmed = bool(bus_sections)
        # With both options, the direct answer compares them (times, faster one).
        concrete_direct = (
            _metro_bus_direct_answer(metro_response, "\n\n".join(bus_sections), language)
        ...
        direct_line = concrete_direct or direct_line
    ...
    def _build_mode_constrained_route_response(
        ...
        surface_alternative_request = bool(
            ...
            and not preferences["exclude_bus"]
            and not preferences["exclude_tram"]
        )
        ...
        checked_cp_without_bus = False
        if preferences["exclude_bus"] and not preferences["exclude_train"]:
            train_tool = self._get_tool_by_name("plan_train_trip")
            train_result = ""
            if train_tool:
                checked_cp_without_bus = True
                train_result = str(
                    self._invoke_tool(
                        train_tool,
                        {"origin": origin, "destination": destination},
            ...
            if train_result and not _tool_result_indicates_no_match(train_result):
                ...
                if language == "pt":
                    return "\n".join(
                        [
                            f"### 🚆 **{origin_display} → {destination_display} sem autocarro**",
                            "",
                            "✅ **Resposta direta:** encontrei uma alternativa suportada por **CP suburbano/AML** sem usar autocarros.",
                            "",
                            "---",
                            "",
                            "### 🚆 **Opção CP**",
                ...
                return "\n".join(
                    [
                        f"### 🚆 **{origin_display} → {destination_display} without buses**",
                        "",
                        "✅ **Direct answer:** I found a supported **CP suburban rail** alternative without buses.",
                        "",
                        "---",
                        "",
                        "### 🚆 **CP option**",
        ...
        if preferences["bus_only"] or preferences["exclude_tram"]:
            ...
            metropolitan_direct = _metropolitana_direct_sentence(metropolitan_block, language) if (
                metropolitan_result and not _tool_result_indicates_no_match(metropolitan_result)
            ) else ""
            if prefer_metropolitana and metropolitan_direct and urban_bus_markdown:
                # A suburban trip ("Cacilhas → Cristo Rei") answered by a direct
                # suburban line: a Lisbon city match is a namesake, not an option.
                sections = [section for section in sections if not section.startswith(f"#### 🚌 {carris_urban_label}")]
                urban_bus_markdown = ""
            ...
            intro = (
                f"### 🚌 Bus-only options for {origin_display} → {destination_display}"
                if language != "pt"
                else f"### 🚌 Opções apenas de autocarro para {origin_display} → {destination_display}"
            )
            option_count = _count_formatted_carris_options(urban_bus_markdown)
            if metropolitan_block and metropolitan_result and not _tool_result_indicates_no_match(metropolitan_result):
                option_count += max(1, _count_formatted_carris_options(metropolitan_block))
            asks_other_same_mode = bool(
                re.search(
                    r"\b(?:outros?|outras?|mais)\s+(?:autocarros?|linhas?\s+de\s+autocarro)\b",
                    _normalize_token(user_message),
                )
                if language == "pt"
                else re.search(
                    r"\b(?:another|other|more)\s+(?:bus|buses|bus\s+lines?)\b",
                    _normalize_token(user_message),
                )
            )
            if asks_other_same_mode and option_count <= 1:
                direct_line = (
                    f"✅ **Direct answer:** I found only **one confirmed bus option** between **{origin_display}** and **{destination_display}** in the available operator data; I will not invent other lines."
                    if language != "pt"
                    else f"✅ **Resposta direta:** só confirmei **uma opção de autocarro** entre **{origin_display}** e **{destination_display}** nos dados disponíveis dos operadores; não vou inventar outras linhas."
                )
            else:
                direct_line = (
                    f"✅ **Direct answer:** I found bus-only options between **{origin_display}** and **{destination_display}** in the available operator data."
                    if language != "pt"
                    else f"✅ **Resposta direta:** encontrei opções apenas de autocarro entre **{origin_display}** e **{destination_display}** nos dados disponíveis dos operadores."
                )
            recommended_option = _summarize_recommended_carris_option(
                urban_bus_markdown,
                language,
            ...
            if recommended_option and not (asks_other_same_mode and option_count <= 1):
                # The best option (and its next departure) is the direct answer.
                direct_line, recommended_option = _direct_line_from_recommended_option(recommended_option, language)
            elif metropolitan_direct and not recommended_option:
                direct_line = f"✅ **{'Resposta direta' if language == 'pt' else 'Direct answer'}:** {metropolitan_direct}"
        ...
        if preferences["exclude_metro"]:
            ...
            if not sections:
                cp_leg_response = self._build_cp_leg_without_metro_access_limitation(
                    origin,
                    destination,
                    user_message,
                    language,
        ...
        if preferences["tram_only"] or preferences["exclude_bus"]:
            ...
            if preferences["tram_only"] and not preferences["exclude_bus"]:
                if language == "pt":
                    return "\n".join(
                        [
                            f"### 🚋 **{origin} → {destination} apenas de elétrico**",
                            "",
                            "⚠️ **Resposta direta:** não consegui confirmar uma rota apenas de **elétrico** para esta ligação nos dados disponíveis da Carris.",
                            "",
                            "---",
                            "",
                            "- 🚋 Verifiquei as opções de elétrico, mas não apareceu uma ligação utilizável para o trajeto pedido.",
                            "- Não substituí por autocarro, Metro ou CP porque pediste especificamente elétrico.",
                            "",
                            self._build_transport_source_line(
                                language,
                                ["[*Carris*](https://www.carris.pt)"],
                ...
                return "\n".join(
                    [
                        f"### 🚋 **{origin} → {destination} by tram only**",
                        "",
                        "⚠️ **Direct answer:** I could not confirm a **tram-only** route for this trip in the available Carris data.",
                        "",
                        "---",
                        "",
                        "- 🚋 I checked tram options, but no usable connection appeared for the requested trip.",
                        "- I did not replace it with bus, Metro, or CP because you specifically asked for tram.",
                        "",
                        self._build_transport_source_line(
                            language,
                            ["[*Carris*](https://www.carris.pt)"],
                ...
                return (
                    "\n".join(
                        [
                            f"### 🚇🚆🚋 **{origin_display} → {destination_display} sem autocarro**",
                            "",
                            "⚠️ **Resposta direta:** não consegui confirmar uma alternativa completa **sem autocarros** com os dados disponíveis.",
                            "",
                            "---",
                            "",
                            "### 🔎 **Verificações feitas**",
                            "",
                            "- 🚇 **Metro de Lisboa:** não ficou confirmada uma rota completa de Metro para esta origem/destino.",
                            "- 🚋 **Elétrico Carris:** não ficou confirmada uma ligação de elétrico utilizável para este trajeto.",
                            (
                                "- 🚆 **CP suburbano/AML:** não ficou confirmada uma ligação ferroviária direta para esta origem/destino."
                                if checked_cp_without_bus
                                else "- 🚆 **CP suburbano/AML:** não foi consultada porque o pedido não abriu uma alternativa ferroviária suportada."
                            ),
                            "",
                            "### ℹ️ **Limitação**",
                            "",
                            "- Como pediste **sem autocarros**, não reaproveitei a opção de autocarro mesmo que ela exista nos dados de superfície.",
                            "- Não vou inventar transbordos, linhas ou tempos que não tenham sido confirmados pelos dados disponíveis.",
                            "",
                            self._build_transport_source_line(language, source_links),
                        ]
                    ).strip()
            ...
            return (
                "\n".join(
                    [
                        f"### 🚇🚆🚋 **{origin_display} → {destination_display} without buses**",
                        "",
                        "⚠️ **Direct answer:** I could not confirm a complete alternative **without buses** with the available data.",
                        "",
                        "---",
                        "",
                        "### 🔎 **Checks made**",
                        "",
                        "- 🚇 **Metro de Lisboa:** no complete Metro route was confirmed for this origin/destination.",
                        "- 🚋 **Carris tram:** no usable tram connection was confirmed for this trip.",
                        (
                            "- 🚆 **CP suburban rail:** no direct rail connection was confirmed for this origin/destination."
                            if checked_cp_without_bus
                            else "- 🚆 **CP suburban rail:** not checked because the request did not open a supported rail alternative."
                        ),
                        "",
                        "### ℹ️ **Limitation**",
                        "",
                        "- Because you asked for **no buses**, I did not reuse a bus option even if one exists in the surface-transport data.",
                        "- I will not invent transfers, lines, or timings that were not confirmed by the available data.",
                        "",
                        self._build_transport_source_line(language, source_links),
                    ]
                ).strip()
    ...
    @staticmethod
    def _generic_destination_clarification(user_message: str, language: str) -> Optional[str]:
        """Ask which place is meant when a route names only a kind of place.

        "Como vou para a Estação?" names no station and no start point; a
        route to one guessed station, or a network overview, answers another
        question. The main stations are offered instead.
        ...
        Returns:
            The clarification, or ``None`` when the request names a place.
        """
        normalized = _normalize_token(user_message).strip(" ?!.")
        if not re.fullmatch(
            r"(?:como\s+(?:vou|chego|posso\s+ir)|quero\s+ir|how\s+(?:do|can)\s+i\s+get|how\s+do\s+i\s+go)\s+"
            r"(?:para|ate|a|ao|to)\s+(?:a\s+|o\s+|the\s+)?(?:estacao|station|paragem|stop|terminal)",
            normalized,
        ):
            return None
        stations = ["Santa Apolónia", "Oriente", "Rossio", "Cais do Sodré", "Entrecampos", "Sete Rios"]
        if language == "pt":
            options = "\n".join(f"- 🚉 **{name}**" for name in stations)
            return (
                "### 🧭 **Qual estação?**\n\n"
                "✅ **Resposta direta:** preciso de saber a que estação queres ir e de onde partes para te indicar o trajeto.\n\n"
                f"---\n\nEstações principais de Lisboa:\n\n{options}\n\n"
                "💡 Por exemplo: *Como vou do Chiado para a estação do Oriente?*"
            )
        options = "\n".join(f"- 🚉 **{name}**" for name in stations)
        return (
            "### 🧭 **Which station?**\n\n"
            "✅ **Direct answer:** tell me which station you want to reach and where you are starting from, and I will give you the route.\n\n"
            f"---\n\nMain Lisbon stations:\n\n{options}\n\n"
            "💡 For example: *How do I get from Chiado to Oriente station?*"
        )

    @staticmethod
    def _supported_alternative_for_unsupported_mode(user_message: str, language: str) -> Optional[str]:
        """Answer a trip by an unsupported network with the supported networks' route.

        "Próximo barco do Cais do Sodré para o Seixal?": the ferry is outside
        the data LISBOA reads, but the same trip by Metro, Carris, Carris
        Metropolitana, or CP can be checked. The answer says the ferry is not
        covered, points to its operator, and gives the supported route.

        Args:
            user_message: The request naming an unsupported network.
            language: Output language (``pt`` or ``en``).

        Returns:
            The combined answer, or ``None`` when the request names no trip
            (origin and destination) or no supported route was found.
        """
        modes = _detect_unsupported_transport_modes(user_message)
        if not modes or not set(modes) <= {"ferries", "fertagus"}:
            return None
        endpoints = _extract_route_endpoints(user_message)
        if not endpoints or len(endpoints) < 2:
            return None
        origin, destination = endpoints[0], endpoints[1]
        is_pt = language == "pt"
        route_prompt = (
            f"Como vou de transportes públicos de {origin} para {destination}?"
            if is_pt
            else f"How do I get from {origin} to {destination} by public transport?"
        )
        try:
            supported = _build_deterministic_route_tool_response(route_prompt)
        except Exception as exc:
            logger.debug("Supported alternative lookup failed for %r: %s", route_prompt, exc)
            return None
        if not supported or _tool_result_indicates_no_match(supported):
            return None
        direct = re.search(r"(?m)^✅\s+\*\*(?:Resposta direta|Direct answer):\*\*\s*(?P<text>.+)$", supported)
        body = _strip_embedded_transport_route_block(supported)
        footer = next((line for line in supported.splitlines() if re.match(r"^\s*📌\s+\*\*(?:Fonte|Source):", line)), "")
        operator = "Transtejo/Soflusa" if "ferries" in modes else "Fertagus"
        operator_link = "[*Transtejo/Soflusa*](https://ttsl.pt)" if "ferries" in modes else "[*Fertagus*](https://www.fertagus.pt)"
        alternative_text = direct.group("text").strip() if direct else ""
        if is_pt:
            opening = (
                f"✅ **Resposta direta:** não tenho os horários dos {'barcos' if 'ferries' in modes else 'comboios'} "
                f"**{operator}**, por isso não confirmo a próxima partida; com as redes que o LISBOA cobre, "
                + (alternative_text[0].lower() + alternative_text[1:] if alternative_text else "a opção está abaixo.")
            )
            note = f"- ℹ️ **{operator}:** confirma horários e partidas diretamente no operador."
        else:
            opening = (
                f"✅ **Direct answer:** I do not have **{operator}** timetables, so I cannot confirm the next departure; "
                "with the networks LISBOA covers, "
                + (alternative_text[0].lower() + alternative_text[1:] if alternative_text else "the option is below.")
            )
            note = f"- ℹ️ **{operator}:** check timetables and departures with the operator."
        title = f"### 🧭 **{origin} → {destination}**"
        if footer:
            footer = re.sub(r"(\*\*(?:Fonte|Source):\*\*\s*)", rf"\1{operator_link} | ", footer, count=1)
        return "\n\n".join(part for part in [title, opening, "---", body, note, footer] if part).strip()
    ...
    def _resolve_deterministic_response(
        ...
        if unsupported_mode_response:
            alternative = self._supported_alternative_for_unsupported_mode(user_message, resolved_language)
            return self._finalize_transport_response(
                alternative or unsupported_mode_response,
                user_message=user_message,
                language=resolved_language,
            )

        generic_destination = self._generic_destination_clarification(user_message, resolved_language)
        if generic_destination:
            return generic_destination

        madeira_metro_ambiguity = self._build_madeira_metro_destination_ambiguity_response(
            user_message=user_message,
            language=resolved_language,
        )
        if madeira_metro_ambiguity:
            return self._finalize_transport_response(
                madeira_metro_ambiguity,
                user_message=user_message,
                language=resolved_language,
            )
        ...
        # "Metro or bus?" names two modes to compare: the single-tool shortcut
        # picked from the word "bus" would answer with buses only.
        compares_modes = bool(
            endpoints
            and _parse_route_mode_preferences(user_message).get("alternative_mode_request")
            and len(_requested_route_option_modes(user_message)) >= 2
        )
        if deterministic_tool_name in non_route_tool_names or (
            deterministic_tool_name in _carris_urban_direct_tools and not compares_modes
        ):
        ...
        route_preferences = _parse_route_mode_preferences(user_message)
        has_route_mode_constraints = _query_has_route_mode_constraints(user_message)
        sequence_route_response = (
            None
            if has_route_mode_constraints
            else _build_deterministic_sequence_route_response(
                user_message,
                resolved_language,
            )
        )
        if sequence_route_response:
            try:
                from agent.agents.planner_agent import _requested_anchor_labels

                sequence_labels = _requested_anchor_labels(user_message)
            except Exception:
                sequence_labels = []
            for origin, destination in zip(sequence_labels, sequence_labels[1:5]):
                self._record_tool_call(
                    "get_route_between_stations",
                    {"origin": origin, "destination": destination},
                )
                if _route_response_uses_carris_urban(sequence_route_response):
                    self._record_tool_call(
                        "carris_find_routes_between",
                        {"origin": origin, "destination": destination},
                    )
        ...
        if not quality_sensitive_route and not route_preferences.get("alternative_mode_request"):
        ...
        if explicit_train_context and _query_asks_cp_airport_access(user_message):
            return self._finalize_transport_response(
                _build_cp_airport_access_response(resolved_language),
                user_message=user_message,
                language=resolved_language,
            )
        ...
        surface_only_route_modes = bool(
            requested_route_modes
            and requested_route_modes <= {"bus", "tram"}
            and not route_preferences["exclude_bus"]
            and not route_preferences["exclude_tram"]
        )
        if (
            endpoints
            and quality_sensitive_route
            and _query_prioritizes_less_walking(user_message)
            and not _query_has_route_mode_constraints(user_message)
        ):
            low_walk_response = self._build_less_walking_carris_route_response(
                endpoints[0],
                endpoints[1],
                user_message,
                resolved_language,
            )
            if low_walk_response:
                finalized_response = self._finalize_transport_response(
                    low_walk_response,
                    user_message=user_message,
                    language=resolved_language,
    ...
    def _invoke_deterministic_tool_call(
        ...
        if (
            tool_name == "carris_find_routes_between"
            and (not result_str or result_str.startswith(("❌", "Error:", "No route")))
        ):
            route_code = _extract_route_code(user_message)
            if route_code:
                routes_tool = self._get_tool_by_name("carris_get_routes")
                if routes_tool:
                    fallback_result = self._invoke_tool(
                        routes_tool,
                        {"route_id": route_code},
                        tool_name="carris_get_routes",
                    )
                    fallback_str = str(fallback_result).strip()
                    if fallback_str and not fallback_str.startswith(("❌", "Error:")):
                        result_str = fallback_str
                        tool_name = "carris_get_routes"
                        tool_args = {"route_id": route_code}

        if tool_name == "carris_find_routes_between":
            route_code = _extract_route_code(user_message)
            if route_code and not re.search(rf"\b(?:Linha|Line)\s+{re.escape(route_code)}\b|\b{re.escape(route_code)}\b", result_str, flags=re.IGNORECASE):
                if resolved_language == "pt":
                    ...
                    note = (
                        f"⚠️ **Note on line {route_code}:** I could not confirm **{route_code}** "
                        "for this origin/destination in the available Carris data, so I show confirmed Carris alternatives."
                    )
                direct_re = r"(?m)^(?:✅|✅)\s+\*\*(?:Resposta direta|Direct answer):\*\*[^\n]+$"
                if re.search(direct_re, result_str):
                    result_str = re.sub(direct_re, lambda match: f"{match.group(0)}\n\n{note}", result_str, count=1)
                else:
                    result_str = f"{note}\n\n{result_str}"
        ...
        if tool_name == "carris_find_routes_between":
            route_code = _extract_route_code(user_message)
            if route_code and not re.search(
                rf"\b(?:Linha|Line)\s+{re.escape(route_code)}\b|\b{re.escape(route_code)}\b",
                finalized_response,
                flags=re.IGNORECASE,
            ):
                if resolved_language == "pt":
                ...
                direct_re = r"(?m)^✅\s+\*\*(?:Resposta direta|Direct answer):\*\*[^\n]+$"
                if re.search(direct_re, finalized_response):
                    finalized_response = re.sub(
                        direct_re,
    ...
    def _infer_route_operators_from_response(response: str) -> List[str]:
        ...
        raw = "\n".join(
            ...
            if not re.match(r"^\s*📌\s*\*\*(?:Fonte|Source):\*\*", line.strip(), flags=re.IGNORECASE)
        )
    ...
    def _finalize_transport_response(
        ...
        *,
        ...
        finalized = _ensure_transport_direct_answer_block(finalized, user_message, language)
        finalized = preserve_contextual_destination_name(finalized, user_message, language)
        finalized = _drop_live_departures_for_another_day(finalized, user_message, language)
        ...
        if operators_used:
            canonical_order = ["metro", "carris", "carris_metropolitana", "cp"]
            operators_used = [operator for operator in canonical_order if operator in operators_used]
        if operators_used:
        ...
        elif _query_asks_cp_airport_access(user_message):
            pass
        else:
    ...
    def invoke(
        ...
        self._active_user_message = effective_user_message
        ...
        response = self.execute_react_loop(
            ...
            tool_enforcement_msg="" if is_greeting else (
                "You MUST use at least one transport tool to get real data. "
                "Do NOT answer from your knowledge base. "
                "For bus/tram queries, prefer carris_find_routes_between or carris_get_routes. "
                "For metro queries, use get_route_between_stations or get_metro_status. "
                "Call the most relevant tool for the user's specific request."
            ),
        )
